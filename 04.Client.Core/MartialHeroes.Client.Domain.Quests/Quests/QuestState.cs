...
{
    /// <summary>
    ///     The accept gate threshold: accept is blocked (absent the bypass) when the level word is at or above this.
    ///     spec: quests.md §4.3 / §10 (26).
    /// </summary>
    ...
    {
        if (!apply) return this; // spec: quests.md §7.1 (handler acts only when apply == 1).
