...
///     880-byte SpawnDescriptor + 96-byte stats block + flag byte);
///     <see cref="ApplicationUseCases.SelectCharacterAsync" />
...
public sealed class CharacterSelectionStore
{
    ...
    public void Retain(in CharacterSlotRecord record)
    {
        if ((uint)record.SlotIndex >=
            MaxSlots) return; // hard 5-slot bound: ignore any slot index beyond 4. spec: §3.2.
    ...
    public SelectOutcome Confirm(int slotIndex)
    {
        ...
        if (record is null) return SelectOutcome.Invalid; // no record retained for an unoccupied/unknown slot.
