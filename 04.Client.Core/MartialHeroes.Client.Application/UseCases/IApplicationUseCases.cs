...
public interface IApplicationUseCases
{
    ...
    /// </summary>
    /// <param name="username">
    ///     Account name. Staged for the flow; written into the 0x2B-prefixed
    ///     plaintext pre-image of the secure 1/4 credential reply.
    /// </param>
    ...
    ///     <see cref="SkillCastValidator" /> + <see cref="CooldownTable" />, and only on
    ...
    ///     client-side §4.2 gates via Domain <see cref="EquipRules.CheckEquip" /> and only on
    ///     <see cref="EquipCheckResult.Allowed" /> sends the 12-byte 2/16 CmsgEquipChange.
    ...
    ///     <see cref="ItemStackOps" /> / <see cref="InventoryGrid" />, then —
    ...
    ///     <see cref="TradeSession" /> state machine, and on acceptance of the request edge
