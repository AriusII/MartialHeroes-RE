...
public sealed class AccountCharacterState
{
    ...
    public AccountCharacterState(int initialCount = 0) => CharacterCount = Clamp(initialCount);
    ...
    /// </summary>
    public void Set(int count) => CharacterCount = Clamp(count);
    ...
    private static int Clamp(int value) => Math.Clamp(value, 0, MaxCount);
}