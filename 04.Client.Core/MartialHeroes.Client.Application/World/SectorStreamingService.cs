...
public sealed class SectorStreamingService
{
    ...
    public bool IsResident((int MapX, int MapZ) sector) => _resident.Contains(sector);
    ...
    {
        (float worldX, _, float worldZ) = position.ToVector3Float();
        (int mapX, int mapZ) = SectorGrid.WorldToSector(worldX, worldZ);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        int ringRadius = SectorGrid.RingRadiusFor(Quality);
        int required = SectorGrid.RequiredSectors(centerMapX, centerMapZ, ringRadius, _ringScratch);

        for (int i = 0; i < required; i++)
        {
            (int MapX, int MapZ) sector = _ringScratch[i];
    ...
    private void EvictOutOfRange(int centerMapX, int centerMapZ)
    {
        ...
        foreach ((int MapX, int MapZ) sector in toEvict)
        {
