...
/// the <see cref="MartialHeroes.Client.Domain.Actors.Actor"/> aggregate: the 240-slot skill hotbar, the
...
public sealed class LocalPlayerState
{
    ...
    public SkillId HotbarSkill(int slot) => _hotbar[slot];

    /// <summary>The skill-point allocation for hotbar <paramref name="slot"/>.</summary>
    public short HotbarPoints(int slot) => _hotbarPoints[slot];
    ...
    {
        if ((uint)slot >= (uint)_hotbar.Length)
        {
