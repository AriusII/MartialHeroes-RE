...
public sealed class GameEngineLoop
{
        ...
        LocalPlayerState? localPlayer = null)
    {
        ...
        if (BaseFixedDeltaMs == 0) BaseFixedDeltaMs = 1; // guard absurdly high rates so a tick still advances time.
    }
    ...
    private uint ScaledDeltaMs()
    {
        ...
        if (double.IsNaN(scale) ||
            scale <= 0.0) return 0u; // paused / invalid scale advances no time but still ticks the loop.
    ...
    {
        if (deltaMs == 0) return; // paused: no simulation advance, snapshot still emitted for a steady stream.
