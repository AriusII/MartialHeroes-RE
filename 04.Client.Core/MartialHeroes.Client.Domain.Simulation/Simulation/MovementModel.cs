...
///         (Q16.16 units/second), exactly as <see cref="Actor.MoveSpeedRawPerSecond" /> does. spec: camera_movement.md
///         §B.5.
///     </para>
/// </remarks>
...
{
    /// <summary>
    ///     Max advance per move-issue when far from the target (real units; squared = 144.0). spec: camera_movement.md
    ///     §B.2 step 3 / §B.5 (12.0).
    /// </summary>
    ...
    /// <summary>
    ///     Move dead-zone radius (real units; squared = 4.0 → 2 units). spec: camera_movement.md §B.2 step 4 / §B.5
    ///     (dead-zone sq 4.0).
    /// </summary>
    public const int MoveDeadZoneUnits = 2;

    /// <summary>
    ///     Squared move dead-zone (2² = 4). Moves whose squared distance is ≤ this are ignored. spec: camera_movement.md
    ///     §B.2 step 4 / §B.5 (sq 4.0).
    /// </summary>
        ...
        in Vector3Fixed rawTarget)
    {
        ...
        var oneSquared = (long)Vector3Fixed.One * Vector3Fixed.One; // raw units per (unit²)
