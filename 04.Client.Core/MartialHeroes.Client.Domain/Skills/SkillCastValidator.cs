...
public static class SkillCastValidator
{
        ...
        long now)
    {
        ...
        if (!caster.PartyRelationAllied)
        {
            return SkillCastResult.PartyRelation;
        }

        // Gate 2: billing / rank. spec: §2.1 gate 2 (code 1).
        if (!caster.BillingRankOk)
        {
            return SkillCastResult.BillingOrRank;
        }

        // Gate 3: busy / already casting. spec: §2.1 gate 3 (code 13).
        if (caster.IsBusyCasting)
        {
        ...
        if (caster.IsMounted)
        {
            return SkillCastResult.MountedOrUnresolved;
        }

        // Gate 5: map / zone-mode. spec: §2.1 gate 5 (code 16).
        if (caster.MapModeForbidsCasting)
        {
            return SkillCastResult.MapModeForbidden;
        }

        // Gate 6: stun / silence. spec: §2.1 gate 6 (code 19).
        if (caster.IsStunnedOrSilenced)
        {
            return SkillCastResult.StunnedOrSilenced;
        }

        // Gate 7: alive. spec: §2.1 gate 7 (code 2).
        if (!caster.IsAlive)
        {
            return SkillCastResult.NotAlive;
        }

        // Gate 8: action-lock. spec: §2.1 gate 8 (code 20).
        if (caster.IsActionLocked)
        {
            return SkillCastResult.ActionLocked;
        }

        // Gate 9: current-target hostile-state. spec: §2.1 gate 9 (code 3).
        if (caster.TargetInBlockingHostileState)
        {
            return SkillCastResult.TargetHostileState;
        }

        // Gate 11: weapon / stance requirement (only when the check is active). spec: §2.1 gate 11 (code 18).
        if (skill.WeaponReqActive && !caster.WeaponRequirementSatisfied)
        {
            return SkillCastResult.WeaponRequirement;
        }

        // Gate 12: self-cast eligibility. spec: §2.1 gate 12 (code 5).
        if (!caster.SelfCastEligible)
        {
            return SkillCastResult.SelfCastIneligible;
        }
        ...
        long mpRequired = (long)SkillDefinition.MpGateMultiplier * skill.MpCostFactor;
        if (caster.AvailableMp < mpRequired)
        {
            return SkillCastResult.NotEnoughMp;
        }
        ...
        SkillCastResult rangeResult = CheckRangeAndLineOfSight(in skill, targeting, in aimPoint);
        if (rangeResult != SkillCastResult.Ok)
        {
            return rangeResult;
        }

        // Gate 17: cast-window timing. spec: §2.1 gate 17 (code 11).
        if (!caster.CastWindowOpen)
        {
            return SkillCastResult.CastWindowTiming;
        }
        ...
        if (!caster.HasTargets && !IsImplicitlyTargeted(skill.TargetMode))
        {
            return SkillCastResult.NoTargets;
        }
        ...
        in Vector3Fixed aimPoint)
    {
        ...
        float effectiveRange = EffectiveRange(in skill, targeting);
        ...
        float squaredDistance = targeting.SquaredPlanarDistanceToAim(in aimPoint);
        if (squaredDistance > effectiveRangeSquared)
        {
            return SkillCastResult.MoveCloser;
        }

        // Terrain / LoS test → code 9 if blocked. spec: §2.2.
        if (!targeting.HasLineOfSight(in aimPoint))
        {
            return SkillCastResult.LineOfSightBlocked;
        }
    ...
    {
        ArgumentNullException.ThrowIfNull(targeting);
        float range = skill.BaseRange + targeting.CasterBodyRadius + targeting.BuffRangeBonus;
    ...
    private static bool IsImplicitlyTargeted(SkillTargetMode mode) =>
        mode is SkillTargetMode.SingleSelfOrPrimary
