...
public readonly record struct BuffDebuff
{
    ...
    /// <summary>Remaining duration in ticks (second dword). Active while &gt; 0; decrements by 1 per tick. spec: skills.md §6.1 / §6.3.</summary>
    public int DurationTicks { get; init; }

    /// <summary>The slot's param tail (third dword). spec: skills.md §6.1.</summary>
    public int Param { get; init; }

    /// <summary>The buff's strength / percent from the parallel secondary table (the %-gated magnitude). spec: skills.md §6.1 / §6.2.</summary>
    public ushort Magnitude { get; init; }

    /// <summary>An empty (cleared) slot.</summary>
    public static BuffDebuff Empty => default;

    /// <summary>True when the slot is active (duration &gt; 0). spec: skills.md §6.2 ("active when its value/duration field is &gt; 0").</summary>
    ...
    /// <summary>The %-gate threshold for effect code 64 (flag set only when magnitude &lt; 100). spec: skills.md §6.2 (64 / 0x40).</summary>
    ...
    public (BuffDebuff Next, bool Expired) TickOnce()
    {
        ...
        if (DurationTicks <= 0)
        {
            return (DurationTicks < 0 ? this with { DurationTicks = 0 } : this, false);
        }

        int next = DurationTicks - 1;
        bool expired = next == 0;
