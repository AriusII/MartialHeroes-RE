...
public readonly record struct ProgressionState
{
    ...
    public ProgressionState AddExperience(long amount) =>
        this with { Experience = Experience.AddExperience(amount) };
    ...
    /// </summary>
    /// <exception cref="LevelTableException">When the divisor for <paramref name="levelCache"/> is 0. spec: progression.md §4.</exception>
    public ProgressionState AddRankXp(
        ...
        this with { RankXp = RankXp.ApplyRankGain(amount, mode, levelCache, divisorTable, capTable) };
}