...
public readonly record struct ExperienceModel
{
    ...
    public ExperienceModel AddExperience(long amount) =>
        this with { CurrentXp = unchecked(CurrentXp + amount), LifetimeXp = unchecked(LifetimeXp + amount) };
    ...
    /// </summary>
    public ExperienceModel ResyncCurrentXp(long currentXp) => this with { CurrentXp = currentXp };
    ...
    /// <param name="ratePercent">The server-set XP bonus rate, as a percentage. Negative rates are clamped
    /// away so the denominator <c>(rate + 100)</c> stays positive; a non-positive denominator yields the
    /// full amount as the base with no bonus.</param>
    public static (long ShownBase, long Bonus) SplitBonus(long amount, long ratePercent)
    {
        ...
        long denominator = ratePercent + 100L;
        if (denominator <= 0L)
        {
            return (amount, 0L);
        }

        long shownBase = checked(100L * amount) / denominator;
