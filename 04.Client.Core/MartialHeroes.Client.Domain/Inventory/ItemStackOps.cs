...
public static class ItemStackOps
{
    ...
    public static bool Split(InventoryGrid grid, int fromIndex, int toIndex, uint quantity)
    {
        ...
        InventorySlot source = grid[fromIndex];
        if (source.IsEmpty || quantity == 0 || quantity >= source.Quantity)
        {
            return false; // nothing to split, or a "split" that would move the whole stack.
        }

        if (!grid.IsEmpty(toIndex))
        {
            return false; // a split targets an empty slot only. spec: inventory_trade.md §9.
        }

        if (quantity > grid.MaxStackSize)
        {
            return false; // the new stack must fit one slot.
        }
