...
public readonly record struct AnimRotation(float X, float Y, float Z, float W)
{
    ...
    public AnimRotation Negated() => new(-X, -Y, -Z, -W);
...
/// <item>The next index is <c>frame + 1</c>, clamped to the last keyframe (clamp-to-last at clip
///   end; looping is a higher-layer concern and is not modelled here).</item>
/// <item>The interpolation parameter <c>alpha = time − frame / 10</c> is passed in <b>raw seconds</b>
...
///   "It is not re-normalized to [0, 1]").</item>
/// </list>
/// </para>
/// <para>
...
public static class AnimationTimeline
{
    ...
    public static (int Lower, int Upper, float Alpha) ResolveFrame(float timeSeconds, int keyCount)
    {
        ...
        int sampleIndex = (int)MathF.Floor(timeSeconds * FramesPerSecond);
        ...
        int lower = sampleIndex;
        int upper = lower + 1 < keyCount ? lower + 1 : keyCount - 1;

        // Raw-seconds alpha, NOT normalised. spec: animation.md §Timing.
        float alpha = timeSeconds - lower * SecondsPerFrame;
        ...
        ReadOnlySpan<AnimRotation> rotations)
    {
        ...
        (int lower, int upper, float alpha) = ResolveFrame(timeSeconds, translations.Length);

        if (lower == upper)
        {
            return new AnimPose(translations[lower], rotations[lower]);
        }

        AnimTranslation t = AnimTranslation.Lerp(translations[lower], translations[upper], alpha);
        AnimRotation r = AnimRotation.Slerp(rotations[lower], rotations[upper], alpha);
