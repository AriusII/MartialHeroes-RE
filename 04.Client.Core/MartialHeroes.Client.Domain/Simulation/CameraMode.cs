...
{
    /// <summary>Over-the-shoulder follow camera (the default). Orbits the player; runs terrain collision. spec: camera_movement.md §A.1 (Third).</summary>
    Third = 0,

    /// <summary>First-person (eye at the player head); yaw/pitch look, no follow distance, no terrain clamp. spec: camera_movement.md §A.1 (First).</summary>
    First = 1,

    /// <summary>Fixed-angle tracking; follows position but never rotates around the player; no terrain collision. spec: camera_movement.md §A.1 (Static).</summary>
    Static = 2,

    /// <summary>Orbit camera for the gamble / betting minigame UI; yaw orbit, no terrain collision. spec: camera_movement.md §A.1 (Gamble).</summary>
    Gamble = 3,
