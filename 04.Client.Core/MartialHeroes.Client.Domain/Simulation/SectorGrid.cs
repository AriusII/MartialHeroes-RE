...
public static class SectorGrid
{
    ...
    public static (int MapX, int MapZ) WorldToSector(float worldX, float worldZ)
    {
        ...
        float adjustedX = worldX < 0.0f ? worldX - SectorSizeWorldUnits : worldX;
        float adjustedZ = worldZ < 0.0f ? worldZ - SectorSizeWorldUnits : worldZ;
        ...
        int cellXRaw = (int)(adjustedX / SectorSizeWorldUnits);
        int cellZRaw = (int)(adjustedZ / SectorSizeWorldUnits);
    ...
    /// </remarks>
    public static int RingRadiusFor(StreamQuality quality) => quality switch
    {
    ...
    {
        int count = RequiredSectorCount(ringRadius);
    ...
    public static int RequiredSectors(
        ...
        Span<(int MapX, int MapZ)> destination) =>
        RequiredSectors(centerX, centerZ, RingRadiusFor(quality), destination);
    ...
    {
        int distX = Math.Abs(cellX - centerX);
        int distZ = Math.Abs(cellZ - centerZ);
