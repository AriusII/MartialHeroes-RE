...
public static class StatAggregation
{
    ...
    /// <c>serverBase + buff(statKey) + all-stats buff(93) + equipment_sum + set_bonus + modifier_slots
    /// + globalAddend</c>. Pure and deterministic. spec: Docs/RE/specs/combat.md §2.
    /// </summary>
    ...
    /// <param name="buffs">Active buff contributions; both the matching per-stat kind and the shared all-stats kind (93) are summed. spec: combat.md §2.2.</param>
    /// <param name="equipment">Per-item equipment grants (slot skips already applied by the caller). spec: combat.md §2.1/§2.3.</param>
    ...
    public static int SumBuffs(StatKey statKey, ReadOnlySpan<BuffContribution> buffs)
    {
        ...
        for (int i = 0; i < buffs.Length; i++)
        {
            ...
            else if (b.Key == StatKey.AllStats && IsPrimaryStat(statKey))
            {
                sum += b.Value;
            }
        }
    ...
    {
        int sum = 0;
        for (int i = 0; i < modifierSlots.Length; i++)
        {
    ...
    public static int SumSetBonus(StatKey statKey, ReadOnlySpan<SetPieceContribution> setPieces)
    {
        ...
        {
            SetPieceContribution piece = setPieces[i];
            if (piece.Key != statKey)
            {
                continue;
            }
            ...
            if (IsSetComplete(piece.SetTypeId, piece.RequiredPieceCount, setPieces))
            {
                sum += piece.SetCompleteBonus;
            }
        }
        ...
        ReadOnlySpan<SetPieceContribution> setPieces)
    {
        ...
        int matched = CountSetPieces(setTypeId, setPieces);
    ...
    private static bool IsPrimaryStat(StatKey statKey) =>
        statKey is StatKey.Str or StatKey.Agi or StatKey.Dex or StatKey.Int or StatKey.Con;
    ...
    public static PrimaryStats AggregatePrimaryStats(
        ...
        ReadOnlySpan<ModifierSlotContribution> modifierSlots)
    {
        int str = Aggregate(StatKey.Str, serverBases.Str, buffs, equipment, setPieces, modifierSlots);
        int dex = Aggregate(StatKey.Dex, serverBases.Dex, buffs, equipment, setPieces, modifierSlots);
        int agi = Aggregate(StatKey.Agi, serverBases.Agi, buffs, equipment, setPieces, modifierSlots);
        int con = Aggregate(StatKey.Con, serverBases.Con, buffs, equipment, setPieces, modifierSlots);
        int @int = Aggregate(StatKey.Int, serverBases.Int, buffs, equipment, setPieces, modifierSlots);
