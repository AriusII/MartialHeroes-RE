...
public static class CombatFormula
{
    ...
    public static int WeaponProficiencyPenaltyPercent(int proficiencyKey) =>
        WeaponProficiencyPenaltyPercent(proficiencyKey, midBandPenalty: 0);
    ...
    public static int WeaponProficiencyPenaltyPercent(int proficiencyKey, int midBandPenalty)
    {
        ...
        if (proficiencyKey >= 75)
        {
            return 100; // spec: combat.md §4 (75 and above → 100)
        }

        if (proficiencyKey >= 31)
        {
            return midBandPenalty; // spec: combat.md §4 (31..74 → 0 or 75; UNVERIFIED #10)
        }

        if (proficiencyKey >= 11)
        {
            return 50; // spec: combat.md §4 (11..30 → 50)
        }

        if (proficiencyKey >= 4)
        {
            return 25; // spec: combat.md §4 (4..10 → 25)
        }
    ...
    public static double ApplyHitPenalty(double value, int penaltyPercent) =>
        value * (1.0 - penaltyPercent / 100.0);
    ...
    /// <c>slot15 + slot94(if≠0) + slot5 + weapon_term + attack_base + grade*0.1 + damage_equip_sum
    /// + level*0.5 + (+2.0 if grade≥8)</c>, then apply <c>(slot83 − 100) %</c> on the running total,
    ...
    /// <c>slot16 + slot20 + weapon_term + secondary_base + grade*0.1 + accuracy_equip_sum + level*0.5
    /// + 300.0 + (300.0 if rank-progress gate set) + (2.0 if grade≥8)</c>, then apply the proficiency
    ...
    public static double ApplyHitPercentMultiplier(double total, int slot83Value)
    {
        if (slot83Value == 0)
        {
            return total; // slot absent: (value − 100) % adjustment is undefined → no-op.
        }
