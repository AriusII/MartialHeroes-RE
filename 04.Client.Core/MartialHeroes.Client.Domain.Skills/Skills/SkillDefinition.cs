...
public readonly record struct SkillDefinition
{
    ...
    /// <summary>
    ///     The combined per-cast hit cap. spec: skills.md §3 ("combined hit count is capped at 40") / §1.4 (max_targets
    ///     clamped to 40).
    /// </summary>
    public const int MaxTargetCap = 40;

    /// <summary>
    ///     Minimum effective range after adding the body radius / buff bonus. spec: skills.md §2.2 ("Clamp to a minimum
    ///     of 1.0").
    /// </summary>
    public const float MinEffectiveRange = 1.0f;

    /// <summary>
    ///     The MP gate multiplier: gate fails when available MP &lt; 100 × mp_cost_factor. spec: skills.md §1.4 (+1332) /
    ///     §2.1 gate 14.
    /// </summary>
    ...
    /// <summary>
    ///     Stamina cost per target (+1370, u16). Total = value × targetCount × buffFactor, floored at value. spec:
    ///     skills.md §1.4 (+1370) / §5.2.
    /// </summary>
    public ushort StaminaCost { get; init; }

    /// <summary>
    ///     Per-skill cooldown duration in milliseconds (<see cref="CooldownCentiseconds" /> × 100). spec: skills.md §4
    ///     (duration = +1334 × 100).
    /// </summary>
    public int CooldownMs => CooldownCentiseconds * 100;

    /// <summary>
    ///     True for the REVIVE category (14): the target must be dead to be valid. spec: skills.md §1.4 (+1306, 14 =
    ///     REVIVE) / §3.1.
    /// </summary>
