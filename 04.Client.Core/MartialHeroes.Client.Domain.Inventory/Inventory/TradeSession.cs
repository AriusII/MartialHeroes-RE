...
public enum TradePhase
{
    ...
    /// <summary>
    ///     Both parties are marked trade-busy (5/106 on); the offer is locked pending confirm. spec: inventory_trade.md
    ///     §8.2/§8.3.
    /// </summary>
    Locked = 3,

    /// <summary>
    ///     The finalize (4/25 phase == 4) committed; items applied. Transient before returning to idle. spec:
    ///     inventory_trade.md §8.3 (FINALIZE).
    /// </summary>
    Committed = 4
}

/// <summary>
///     A deterministic player-to-player trade state machine:
///     <c>
///         offer → window → lock → confirm →
///         commit / cancel
///     </c>
///     , with explicit, total transitions and the §8 invariants (40-slot capacity,
...
{
    /// <summary>
    ///     Trade window max items (the 4/25 item_count cap / 40-entry trade array). spec: inventory_trade.md §8.2 / §10
    ///     (40 / 0x28).
    /// </summary>
    ...
    /// <summary>
    ///     The number of items in the local half of the trade window (0..<see cref="MaxItems" />). spec:
    ///     inventory_trade.md §8.2.
    /// </summary>
    ...
    /// <summary>
    ///     Applies the request result (4/23): <c>result == 1</c> opens the window (
    ///     <c>
    ///         requested →
    ///         window-open
    ///     </c>
    ///     ); any other result cancels back to idle. spec: inventory_trade.md §8.2/§8.3.
    /// </summary>
    public TradeSession OnRequestResult(bool accepted)
    {
        if (Phase != TradePhase.Requested) return this; // total: ignore an out-of-phase result.
