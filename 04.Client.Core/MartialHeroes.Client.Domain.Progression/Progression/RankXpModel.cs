...
///         (§4): <c>rank_acc += (remainder + amount) / divisor[idx]</c> and
///         <c>
///             within = (remainder + amount) %
///             divisor[idx]
///         </c>
///         , where the index is the local-player <em>level cache</em>. The progression caps at 25.
///     </para>
///     <para>
...
{
    /// <summary>
    ///     The §4 rank-progression cap (the level-cache value at which the per-level table is bounded). spec:
    ///     progression.md §4.
    /// </summary>
    ...
    /// <param name="levelCache">
    ///     The local-player level cache — the index into both tables and the value
    ///     the cap special-case tests against 25.
    /// </param>
    /// <param name="divisorTable">Per-level XP-per-rank-step divisors (injected DATA). Indexed by level.</param>
    /// <param name="capTable">
    ///     Per-level within-rank cap values (injected DATA). Indexed by level. May be
    ///     <c>null</c>/empty when the caller does not bound the remainder.
    /// </param>
    /// <exception cref="LevelTableException">
    ///     Thrown when the divisor for <paramref name="levelCache" /> is 0
    ///     (the client's "leveltable error"). spec: progression.md §4.
    /// </exception>
