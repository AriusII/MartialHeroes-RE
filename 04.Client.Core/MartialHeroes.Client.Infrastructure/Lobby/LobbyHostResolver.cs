...
public sealed class LobbyHostResolver : ILobbyHostResolver
{
    ...
    public string Resolve()
    {
        ...
        string ipTxtPath = Path.Combine(_clientRoot, IpTxtFileName);
        if (File.Exists(ipTxtPath))
        {
            string? host = TryReadIpTxt(ipTxtPath);
        ...
        {
            string listDatPath = Path.Combine(_clientRoot, ListDatFileName);
            if (File.Exists(listDatPath))
            {
                string? host = TryReadListDat(listDatPath);
                if (!string.IsNullOrWhiteSpace(host))
                    return host;
            }
        }
    ...
    private static string? TryReadIpTxt(string path)
    {
        try
        {
            ...
            string token = wsIdx < 0 ? text : text[..wsIdx];
            if (token.Length == 0)
                return null;
    ...
    private string? TryReadListDat(string path)
    {
        try
        {
            ...
            long expectedSize = (long)ListDatRecordSize * count + ListDatHeaderSize;
            ...
            string? registryName = TryReadRegistryServerName();
            ...
            {
                int recordStart = ListDatHeaderSize + (int)(i * ListDatRecordSize);

                // The CP949 name fills [recordStart .. recordStart+256) (up to NUL).
                // spec: lobby.yaml §RECORD SHAPE C (+0 CP949 server NAME).
                string recordName = ReadNulTerminatedCp949(data, recordStart, ListDatHostOffset);

                bool matches = registryName is not null
                ...
                string host = ReadNulTerminatedCp949(
                    data,
