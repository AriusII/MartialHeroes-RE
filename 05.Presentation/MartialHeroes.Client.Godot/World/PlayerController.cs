...
public sealed partial class PlayerController : Node3D
{
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        var screenPos = mb.Position;
        var hit = TryRaycastGround(cam, screenPos);
    ...
    public override void _Process(double delta)
    {
        ...
        {
            var toGoal = _destination - _avatar.GlobalPosition;
        ...
        if (moveDir.LengthSquared() > 1e-6f)
        {
            ...
            // Translate avatar.
            var newPos = _avatar.GlobalPosition + moveDir * (WalkSpeed * dt);
            ...
            {
                var lookTarget = _avatar.GlobalPosition + _facing;
    ...
    private Vector3? TryRaycastGround(Camera3D cam, Vector2 screenPos)
    {
        ...
        var rayOrigin = cam.ProjectRayOrigin(screenPos);
        var rayDir = cam.ProjectRayNormal(screenPos);
        ...
        if (space is not null)
        {
            ...
            if (_avatar is not null)
                ...
                foreach (var child in _avatar.GetChildren())
            ...
            {
                var hitPos = posVariant.AsVector3();
        ...
        var t = (GroundY - rayOrigin.Y) / denominator;
        if (t < 0f)
            return null; // intersection is behind the camera

        var planeHit = rayOrigin + rayDir * t;
        return planeHit;
    }
    ...
    ///     <para>
    ///         Uses a signed-angle approach in the XZ plane to guarantee the shortest-path
    ///         rotation (no 360° spins when crossing the ±π boundary).
    ///     </para>
    ...
    private static Vector3 SmoothedFacing(Vector3 current, Vector3 target, float maxAngle)
    {
        ...
        var dot = Mathf.Clamp(current.Dot(target), -1f, 1f);
        var cross = current.X * target.Z - current.Z * target.X; // Y component of cross product

        var signedAngle = Mathf.Atan2(Mathf.Abs(cross), dot) * Mathf.Sign(cross);

        // Clamp so we rotate at most maxAngle this frame.
        var step = Mathf.Clamp(signedAngle, -maxAngle, maxAngle);
