...
//
//   Si le répertoire ou les fichiers sont absents, le monde reste vide
//   (aucun crash, aucune exception propagée à l'utilisateur).
//
...
public sealed class RealClientAssets : IDisposable
{
    ...
    public List<(int MapX, int MapZ)> EnumerateTerrainCells(int areaId)
    {
        ...
        {
            var name = entry.Name; // already lower-case, spec: pak.md §TOC names. CONFIRMED.
