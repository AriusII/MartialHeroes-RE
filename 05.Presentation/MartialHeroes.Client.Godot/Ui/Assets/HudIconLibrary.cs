...
///     <para>
///         All methods return <see langword="null" /> when the VFS is unavailable or an id is absent.
///         Callers MUST handle null and render nothing.
///     </para>
...
public sealed class HudIconLibrary : IDisposable
{
    ...
    ///     <para>
    ///         Call once per session when the player's class and stance are known (e.g. on
    ///         character select or class-change). Loads the .do stance table and resolves the
    ///         512×512 DDS sheet from skillicon.txt.
    ///     </para>
    ///     <para>
    ///         jobId: 1=Musa, 2=Assassin, 3=Wizard, 4=Monk.
    ///         kindId: 1=jung, 2=sa, 3=ma.
    ///     </para>
    ...
    ///     <para>
    ///         Each item icon is a whole-texture blit — the entire DDS at native dimensions.
    ///         No atlas sub-rect is used.
    ///     </para>
    ...
    ///     <para>
    ///         Draw cell size is 21×21 (sprite-sheet-pending — see spec note). Use
    ///         <see cref="BuffIconDrawW" />/<see cref="BuffIconDrawH" /> constants.
    ///     </para>
