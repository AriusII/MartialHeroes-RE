...
public sealed class CryptoOutboundPacketSink : IOutboundPacketSink
{
    ...
    ///     <list type="number">
    ///         <item>
    ///             If payload is empty, send a bare 8-byte header frame from a pooled rental — no cipher,
    ///             no compression (spec: Docs/RE/specs/crypto.md §2 header-only pass-through).
    ///         </item>
    ///         <item>
    ///             Rent ONE buffer (header + worst-case compress bound); copy the plaintext into its
    ///             payload region and cipher it in place.
    ///         </item>
    ///         <item>LZ4-compress the enciphered bytes (the codec returns its own pooled output).</item>
    ///         <item>
    ///             Copy the compressed bytes back over the payload region, write the 8-byte header ahead of
    ///             them in the SAME rental, and forward to <see cref="IConnectionSession.SendAsync" />.
    ///         </item>
    ///     </list>
    /// </remarks>
