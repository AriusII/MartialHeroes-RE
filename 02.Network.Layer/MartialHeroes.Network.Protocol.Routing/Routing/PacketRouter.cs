...
//   ONLY when IPacketHandler has a matching Handle(in T) overload. One typed arm is emitted per
//   Handle(in T) overload on IPacketHandler (every other opcode falls through to the default arm,
//   which calls OnUnhandled). Adding a new Handle(in T) to IPacketHandler automatically adds the
//   arm on the next build with no manual table update.
...
public static partial class PacketRouter
{
    ...
    public static bool Route(uint packedOpcode, ReadOnlySpan<byte> payload, IPacketHandler handler)
    {
        ...
        // Delegate to the source-generated switch (this class's other half, emitted into
        // PacketRouter.g.cs by MartialHeroes.Network.Protocol.Generators). The generator emits a
        // case arm for each [PacketOpcode]-tagged struct that has a Handle(in T) overload in
        // IPacketHandler; all other opcodes reach OnUnhandled via the generated switch's default
        // arm. spec: Docs/RE/opcodes.md.
