...
public readonly struct SmsgGroundItemRemove
{
    ...
    /// <summary>
    ///     0x08 — ground-item entity key to remove (matches spawn Param2) (u32). spec:
    ///     Docs/RE/packets/5-15_ground_item_remove.yaml.
    /// </summary>
    public readonly uint TrackedId;

    /// <summary>
    ///     0x0C — when set, shows a "X picked up Y" notice for party/relation context (u8). spec:
    ///     Docs/RE/packets/5-15_ground_item_remove.yaml.
    /// </summary>
    public readonly byte NotifyFlag;

    /// <summary>
    ///     0x0D — alignment padding to 16 bytes (3 bytes). spec: Docs/RE/packets/5-15_ground_item_remove.yaml (Pad0:
    ///     bytes[3]).
    /// </summary>
