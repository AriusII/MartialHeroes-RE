...
public static class PayloadCompression
{
    ...
    {
        int maxOut = LZ4Codec.MaximumOutputSize(source.Length);
    ...
    {
        byte[] buffer = ArrayPool<byte>.Shared.Rent(InboundMaxDecompressedSize);
        try
        {
    ...
    {
        if (destination.Length > InboundMaxDecompressedSize)
        {
            destination = destination[..InboundMaxDecompressedSize];
        }

        int written = LZ4Codec.Decode(source, destination);
        if (written < 0)
        {
            throw new InvalidOperationException(
