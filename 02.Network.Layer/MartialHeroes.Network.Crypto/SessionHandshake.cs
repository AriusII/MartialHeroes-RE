...
public static class SessionHandshake
{
    ...
    ///     <para>
    ///         Layout (spec §6.2, §6.2.1):
    ///         <c>
    ///             headerA(2) ‖ headerB(2) ‖ [u32 LE L1] ‖ modulus[L1] (big-endian)
    ///             ‖ [u32 LE L2] ‖ exponent[L2] (big-endian) ‖ scalar1(4) ‖ scalar2(4)
    ///         </c>
    ///         . The two 2-byte headers
    ///         are opaque tags — stored nowhere, ignored (§6.2.2).
    ///     </para>
    ///     spec: Docs/RE/specs/crypto.md §6.2.
    /// </summary>
    /// <exception cref="ArgumentException">
    ///     If the payload is malformed (wrong size, lengths that do not honor L1 + L2 = 42, or
    ///     out-of-range fields).
    /// </exception>
    ...
    public readonly struct KeyExchange
    {
        ...
        /// <summary>
        ///     Wire byte width of the modulus digit array (<c>L1</c>). NOTE: NOT the PKCS#1 block size — see
        ///     <see cref="BlockSizeK" />.
        /// </summary>
