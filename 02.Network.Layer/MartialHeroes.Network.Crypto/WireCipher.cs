...
public static class WireCipher
{
    ...
    private static void EncryptForwardSweep(Span<byte> payload)
    {
        ...
        var p = (byte)payload.Length; // remaining-length countdown, low 8 bits. §3.1 load-bearing note

        for (var i = 0; i < payload.Length; i++)
        {
            var t = Rol8(payload[i], ForwardRol); // ROL 3
    ...
    private static void EncryptBackwardSweep(Span<byte> payload)
    {
        ...
        {
            var t = Rol8(payload[i], BackwardRol); // ROL 4
    ...
    private static void DecryptForwardSweep(Span<byte> payload)
    {
        ...
        for (var i = 0; i < payload.Length; i++)
        {
            ...
            var t = Rol8(rored, ForwardRor); // undo ROR8(.,1) → post-feedback intermediate (acc_i)
            var input = (byte)((t ^ accPrev) - p); // undo XOR feedback, then undo ADD8(.,p)
