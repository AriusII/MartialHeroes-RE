...
public static class Opcodes
{
    ...
    /// <summary>
    ///     1:0 — client logout / quit-from-in-game request (header-only, 0 B). status: draft. spec:
    ///     packets/cmsg_logout.yaml.
    /// </summary>
    ...
    ///     packets/cmsg_char_enter.yaml.
    /// </summary>
    public const uint CmsgEnterGameRequest = 0x10009;

    /// <summary>
    ///     1:13 — client rename-character request (18 B: slot + 17 B CP949 name). status: draft. spec:
    ///     packets/cmsg_char_rename.yaml.
    /// </summary>
    ...
    /// <summary>
    ///     2:0 — leave-world / logout-to-lobby request (header-only, 0 B). status: confirmed. spec:
    ///     packets/2-0_leave_world.yaml, Docs/RE/specs/world_exit.md §1.
    /// </summary>
    public const uint CmsgLeaveWorld = 0x20000;

    /// <summary>
    ///     2:7 — client whisper / named private message (19 B header + text). status: draft. spec:
    ///     packets/2-7_whisper.yaml.
    /// </summary>
    public const uint CmsgWhisper = 0x20007;

    /// <summary>
    ///     2:13 — client click-to-move / position-sync request (16 B). status: draft. spec:
    ///     packets/2-13_move_request.yaml.
    /// </summary>
    ...
    /// <summary>
    ///     3:21 — client general / channel chat (56 B header + text). status: draft. spec:
    ///     packets/3-21_chat_channel.yaml.
    /// </summary>
