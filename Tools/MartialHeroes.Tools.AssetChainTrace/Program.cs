...
namespace MartialHeroes.Tools.AssetChainTrace;
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        if (!ResolveClient(infOverride, vfsOverride, out var inf, out var vfs))
        {
        ...
        switch (verb)
        {
            ...
            case "stride":
                if (positional.Count < 3 || !int.TryParse(positional[2], out var rec) || rec <= 0)
                {
    ...
    private static void ReportPath(MappedVfsArchive archive, string vfsPath)
    {
        var norm = vfsPath.Replace('\\', '/').ToLowerInvariant();
        var present = archive.Contains(norm);
        if (!present)
        {
    ...
    private static void ReportStride(MappedVfsArchive archive, string vfsPath, int recordBytes)
    {
        var norm = vfsPath.Replace('\\', '/').ToLowerInvariant();
        ...
        foreach (var e in archive.GetEntries())
        {
            ...
            var size = e.DataSize;
            var count = size / recordBytes;
            var rem = size % recordBytes;
            var flag = rem == 0
                ? ""
    ...
    private static void ReportCensus(MappedVfsArchive archive)
    {
    ...
    private static bool ResolveClient(string? infOverride, string? vfsOverride, out string inf, out string vfs)
    {
        ...
        {
            inf = Config.ClientDataInf;
            vfs = Config.ClientDataVfs;
            return true;
        }

        if (File.Exists(Config.LegacyInf) && File.Exists(Config.LegacyVfs))
        {
            inf = Config.LegacyInf;
            vfs = Config.LegacyVfs;
            return true;
        }
