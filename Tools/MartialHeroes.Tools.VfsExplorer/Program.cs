...
using MartialHeroes.Tools.VfsExplorer;
...
using var archive2 = MappedVfsArchive.Open(infPath, vfsPath);
var entries = archive2.GetEntries();
...
{
    var present = archive2.Contains(containsPath);
...
if (headPath is not null)
{
    ...
    var content = archive2.GetFileContent(headPath);
    var n = Math.Min(headBytes, content.Length);
    var head = content.Span[..n];
...
static int RunScanMot(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    for (var i = 0; i < args.Length - 1; i++)
        if (args[i] == "--id" && uint.TryParse(args[i + 1], out var v))
            idFilter = v;
    ...
    foreach (var e in motEntries)
    {
        ...
        var firstU32 = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span[..4]);
        if (firstU32 == BaniMagic)
        {
            ...
            if (idFilter is not null)
                if (raw.Length >= 16)
                {
                    var version = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(4, 4));
                    var animId = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(8, 4));
                    var unknownField = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(12, 4));
                    Console.WriteLine(
                        $"  BANI {e.Name}: version={version} anim_id={animId} unknown_field={unknownField}");
                }
        ...
        try
        {
            ...
            var isReal = clip.FrameCount > 0 && clip.Tracks.Length > 0;
            if (isReal) realClips++;
            else stubs++;
            ...
            var fcBucket = clip.FrameCount switch
            {
                0 => "0 (stub)",
                <= 10 => "1-10",
                <= 50 => "11-50",
            ...
            frameCountBuckets.TryGetValue(fcBucket, out var fcN);
            frameCountBuckets[fcBucket] = fcN + 1;

            // Bucket track counts: 0, 1-10, 11-30, 31-50, 51+
            var tcBucket = clip.Tracks.Length switch
            {
                0 => "0 (stub)",
                <= 10 => "1-10",
                <= 30 => "11-30",
            ...
            trackCountBuckets.TryGetValue(tcBucket, out var tcN);
...
static int RunScanBnd(MappedVfsArchive archive, string[] args)
{
    ...
    foreach (var e in bndEntries)
        try
        {
            var raw = archive.GetFileContent(e.Name);
            var skel = BndParser.Parse(raw);

            var bc = skel.Bones.Length;
            if (bc <= 1) singleBone++;
            else multiBone++;
            ...
            boneCountBuckets.TryGetValue(bucket, out var bn);
            ...
            if (bc > 0)
            {
                var baseId = (byte)(skel.Bones[0].SelfId & 0xFF);
    ...
    foreach (var (bucket, cnt) in boneCountBuckets)
...
static int RunScanSkn(MappedVfsArchive archive, string[] args)
{
    ...
    foreach (var e in sknEntries)
        try
        {
            ...
            {
                var ratio = (double)mesh.Weights.Length / mesh.Positions.Length;
                var est = (int)Math.Ceiling(ratio);
...
static int RunScanUi(MappedVfsArchive archive, string[] args)
{
    ...
    var byDir =
        new SortedDictionary<string, List<(string name, uint w, uint h, string fourcc, bool npot)>>(StringComparer
            .Ordinal);
    ...
    foreach (var e in archive.GetEntries())
    {
        ...
        var raw = archive.GetFileContent(e.Name);
        if (raw.Length < DdsMinHeaderSize)
        {
            nonDds++;
            continue;
        }

        var magic = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(DdsMagicOff, 4));
        if (magic != DdsMagic)
        {
            nonDds++;
            continue;
        ...
        var h = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(DdsHeightOff, 4));
        var w = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(DdsWidthOff, 4));
        var fourccRaw = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(DdsPfFourCcOff, 4));
        ...
        var npot = !IsPowerOf2(w) || !IsPowerOf2(h);
        if (npot) totalNpot++;
        totalDds++;

        // Group by directory (everything between "data/ui/" and the filename).
        var subdir = "data/ui/";
        var lastSlash = e.Name.LastIndexOf('/');
    ...
    foreach (var (dir, items) in byDir)
    {
        Console.WriteLine($"\n  [{dir}]  ({items.Count} files)");
        foreach (var (name, w, h, fourcc, npot) in items)
        {
            var npotMark = npot ? " *** NPOT ***" : "";
...
static int RunDumpMsgXdb(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    for (var i = 0; i < args.Length - 1; i++)
        if (args[i] == "--id" && uint.TryParse(args[i + 1], out var v))
        {
            filterId = v;
                 ...
                 uint.TryParse(args[i + 1], out var ra) && uint.TryParse(args[i + 2], out var rb))
        {
            rangeA = ra;
            rangeB = rb;
        }
    ...
    var raw = archive.GetFileContent(MsgXdbPath);
    var catalog = MsgXdbParser.Parse(raw);
    ...
    foreach (var rec in catalog.Records)
    {
        ...
        var text = rec.Text.Replace("\r", "\\r", StringComparison.Ordinal)
...
static int RunDumpUitex(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    var raw = archive.GetFileContent(UiTexPath);
    var manifest = UiTexManifestParser.Parse(raw.Span);
    ...
    foreach (var entry in manifest.DdsEntries.Concat(manifest.MskEntries))
    {
        // Check whether the referenced path actually exists in the VFS.
        var vfsLookup = entry.VfsPath.ToLowerInvariant().Replace('\\', '/');
        var exists = archive.Contains(vfsLookup);
        var mark = exists ? "" : "  [NOT IN VFS]";
        var block = entry.BlockKind == UiTexBlockKind.Dds ? "DDS" : "MSK";
...
static int RunScanXeff(MappedVfsArchive archive, string[] args)
{
    ...
    foreach (var e in xeffEntries)
    {
        ...
        // File size buckets.
        var sizeBucket = e.DataSize switch
        {
        ...
        sizeBuckets.TryGetValue(sizeBucket, out var sbn);
        ...
        var effectId = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span[..4]);
        var elementCount = BinaryPrimitives.ReadUInt32LittleEndian(raw.Span.Slice(4, 4));
        ...
        seenIds.TryGetValue(effectId, out var idCount);
        seenIds[effectId] = idCount + 1;

        if (elementCount == 0) emptyEffects++;
        else nonEmptyEffects++;

        var elemBucket = elementCount switch
        {
            0 => "0 (stub/empty)",
            1 => "1",
            <= 5 => "2-5",
        ...
        elemCountBuckets.TryGetValue(elemBucket, out var ebn);
        elemCountBuckets[elemBucket] = ebn + 1;
    }

    var duplicateIds = seenIds.Values.Count(c => c > 1);
    ...
    Console.WriteLine("\n  element_count distribution:");
    foreach (var (b, cnt) in elemCountBuckets)
        Console.WriteLine($"    {b,-22}  {cnt,5:N0}");

    Console.WriteLine("\n  file size distribution (bytes):");
    foreach (var (b, cnt) in sizeBuckets)
...
static int RunScanSound(MappedVfsArchive archive, string[] args)
{
    ...
    foreach (var e in archive.GetEntries())
    {
        ...
        var isEff = e.Name.Contains("/soundtable", StringComparison.Ordinal) &&
                    ext == ".eff";
    ...
    {
        var ext = ExtOf(e.Name);
        tableByExt.TryGetValue(ext, out var n);
        tableByExt[ext] = n + 1;
    }

    foreach (var (ext, cnt) in tableByExt)
    ...
    foreach (var e in tableEntries)
    {
        ...
        var tableBytes = raw.Span[..SoundTableDataSize];

        for (var idx = 0; idx < SoundTableEntryCount; idx++)
        {
            // sound_entry_id u32 LE @ entry+0x00.
            // spec: Docs/RE/formats/sound_tables.md §Per-entry layout — sound_entry_id @ +0x00: CONFIRMED.
            var soundId = BinaryPrimitives.ReadUInt32LittleEndian(
            ...
            var expectedPath3d = $"data/sound/3d/{soundId}.ogg";
            var expectedPath2d = $"data/sound/2d/{soundId}.ogg";
            var exists3d = archive.Contains(expectedPath3d);
            var exists2d = archive.Contains(expectedPath2d);
            var existsMark = exists3d ? "3d-ok" : exists2d ? "2d-ok" : "MISSING";
...
static int RunScanFx(MappedVfsArchive archive, string[] args)
{
    ...
    foreach (var e in archive.GetEntries())
    {
        ...
        var f0 = BinaryPrimitives.ReadUInt32LittleEndian(hdr[..4]);
        ...
        var f3 = BinaryPrimitives.ReadUInt32LittleEndian(hdr[12..16]);
        // field[4] at +0x10 (mesh_count in FX1/FX2): u32 LE
        // spec: Docs/RE/formats/terrain_layers.md §1.5 — mesh_count @ 0x10: CONFIRMED.
        var f4 = BinaryPrimitives.ReadUInt32LittleEndian(hdr[16..20]);
        // field[5] at +0x14 (index_count in FX1/FX2): u32 LE
        // spec: Docs/RE/formats/terrain_layers.md §1.5 — index_count @ 0x14: CONFIRMED.
        var f5 = BinaryPrimitives.ReadUInt32LittleEndian(hdr[20..24]);

        field0Histo[ext].TryGetValue(f0, out var f0n);
        field0Histo[ext][f0] = f0n + 1;
        field3Histo[ext].TryGetValue(f3, out var f3n);
        field3Histo[ext][f3] = f3n + 1;
        field4Histo[ext].TryGetValue(f4, out var f4n);
        field4Histo[ext][f4] = f4n + 1;
        field5Histo[ext].TryGetValue(f5, out var f5n);
        field5Histo[ext][f5] = f5n + 1;
    }
    ...
    foreach (var ext in layerExts)
    {
        ...
        var shown5 = 0;
        foreach (var (v, n) in field5Histo[ext].OrderByDescending(p => p.Value))
        {
            Console.WriteLine($"      {v,6} (0x{v:X4}) × {n,5:N0}");
            if (++shown5 >= 10) break;
        }
...
static int RunDumpDo(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    for (var d = 0; d < NumU32; d++)
    {
        distinctU32[d] = new HashSet<uint>();
        minU32[d] = uint.MaxValue;
    }
    ...
    foreach (var path in doFiles)
    {
        if (!archive.Contains(path))
        {
            Console.WriteLine($"  {Path.GetFileName(path),-22}  MISSING");
            missing++;
            continue;
        }

        var mem = archive.GetFileContent(path);
        var raw = mem.Span;
        var nrec = raw.Length / Stride;
        var tail = raw.Length % Stride;
        totalFiles++;
        totalRecords += nrec;
        ...
        {
            var r = raw.Slice(i * Stride, Stride);
            ...
            var sx = BinaryPrimitives.ReadInt16LittleEndian(r.Slice(0x18, 2));
            var sy = BinaryPrimitives.ReadInt16LittleEndian(r.Slice(0x1C, 2));
            ...
            for (var d = 0; d < NumU32; d++)
            {
                var v = BinaryPrimitives.ReadUInt32LittleEndian(r.Slice(d * 4, 4));
                distinctU32[d].Add(v);
        ...
        var classRefStr = string.Join(",", classRefSet.Take(5)) + (classRefSet.Count > 5 ? "…" : "");
        Console.WriteLine(
            $"  {Path.GetFileName(path),-22}  {raw.Length,10:N0}  {nrec,8:N0}  {tail,5}  {validIcon,10:N0}  [{classRefStr}]");
    }

    Console.WriteLine($"\n  totals: {totalFiles} files, {totalRecords:N0} records, {missing} missing");
    if (totalRecords > 0)
        Console.WriteLine(
            $"  iconSrcX/Y in [0..{MaxCoord}]: {totalValidIcon:N0} / {totalRecords:N0} ({100.0 * totalValidIcon / totalRecords:F1}%)");
    ...
    {
        var o = d * 4;
        var zp = 100.0 * zeroU32[d] / totalRecords;
        var field = o switch
        {
        ...
        Console.WriteLine(
            $"    {o,4:X2}  {distinctU32[d].Count,9:N0}  {minU32[d],12:N0}  {maxU32[d],12:N0}  {zp,5:F1}%  {field}");
    }
...
static int RunScanMinimap(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    {
        var mem = archive.GetFileContent(mapSettingPath);
        var data = mem.Span;
        var zoneCount = data.Length / MapSettingStride;
        var tail = data.Length % MapSettingStride;
        Console.WriteLine(
        ...
        {
            var rec = data.Slice(i * MapSettingStride, MapSettingStride);
            ...
            var id = BinaryPrimitives.ReadInt32LittleEndian(rec[..]);
            var name = cp949.GetString(rec.Slice(4, 36)).TrimEnd('\0');
            var minX = BinaryPrimitives.ReadInt32LittleEndian(rec[0x28..]);
            var minY = BinaryPrimitives.ReadInt32LittleEndian(rec[0x2C..]);
            var maxX = BinaryPrimitives.ReadInt32LittleEndian(rec[0x30..]);
            var maxY = BinaryPrimitives.ReadInt32LittleEndian(rec[0x34..]);
    ...
    foreach (var e in archive.GetEntries())
    {
        var nl = e.Name.ToLowerInvariant();
        ...
        var recs = (int)(e.DataSize / RegionStride);
        var rtail = e.DataSize % RegionStride;
        regionFiles++;
        regionRecords += recs;
    ...
    foreach (var e in archive.GetEntries())
    {
        ...
        var mem = archive.GetFileContent(e.Name);
        if (mem.Length < 0x58) continue;
        var d = mem.Span;
        if (BinaryPrimitives.ReadUInt32LittleEndian(d[..4]) != DdsMagic) continue;

        var h = BinaryPrimitives.ReadUInt32LittleEndian(d[0x0C..]);
        var w = BinaryPrimitives.ReadUInt32LittleEndian(d[0x10..]);
        var fourccRaw = BinaryPrimitives.ReadUInt32LittleEndian(d.Slice(0x54, 4));
        var fourcc = fourccRaw == 0
            ? "RAW"
            : Encoding.ASCII.GetString(BitConverter.GetBytes(fourccRaw)).TrimEnd('\0');
        mapDds++;
...
static int RunScanQuest(MappedVfsArchive archive, string[] args, Encoding cp949)
{
    ...
    static void ScanQuestsScr(MappedVfsArchive archive, string path, int stride, Encoding cp949)
    {
        if (!archive.Contains(path))
        {
            Console.WriteLine($"\nscan-quest: MISSING {path}");
            return;
        }

        var mem = archive.GetFileContent(path);
        var raw = mem.Span;
        var slots = raw.Length / stride;
        var tail = raw.Length % stride;
        ...
        for (var i = 0; i < slots; i++)
        {
            ...
            var id = BinaryPrimitives.ReadUInt16LittleEndian(raw.Slice(i * stride, 2));
            if (id == 0) continue;
            occupied++;
        ...
        Console.WriteLine($"\nscan-quest: {path}");
        Console.WriteLine(
            $"  {raw.Length:N0} bytes, stride {stride}, {slots} slots (tail {tail}), {occupied} occupied (u16 id@0 != 0)");
        if (occupied > 0)
        {
    ...
    static void ScanFixedStride(MappedVfsArchive archive, string path, int stride, Encoding cp949)
    {
        ...
        for (var i = 0; i < nrec && ids.Count < 8; i++)
...
static void PrintHexDump(ReadOnlySpan<byte> data)
{
    ...
    for (var off = 0; off < data.Length; off += width)
    {
        ...
        var end = Math.Min(off + width, data.Length);
        ...
        var produced = end - off;
        var pad = (width - produced) * 3 + (produced <= 8 ? 1 : 0);
