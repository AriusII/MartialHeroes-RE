...
internal static class Commands
{
    ...
    public static int Decode(MappedVfsArchive archive, string[] args, Encoding cp949)
    {
        ...
        var content = archive.GetFileContent(vfsPath);
        var ext = FormatRegistry.ExtOf(vfsPath);
        var entry = FormatRegistry.Lookup(vfsPath, content);
        ...
        try
        {
            foreach (var line in entry.Decode(vfsPath, content))
    ...
    public static int Extract(MappedVfsArchive archive, string[] args)
    {
        ...
        if (!GuardOutputPath(outFile, false, out var reason))
        {
    ...
    public static int Convert(MappedVfsArchive archive, string[] args)
    {
        ...
        if (!GuardOutputPath(outDir, true, out var reason))
        {
        ...
        var stem = Path.GetFileNameWithoutExtension(vfsPath);
        var outFile = Path.Combine(outDir, stem + entry.ConvertOutExt);
        ...
        try
        {
            ...
            var written = new FileInfo(outFile).Length;
    ...
    public static int Hexdump(MappedVfsArchive archive, string[] args, Encoding cp949)
    {
        ...
        var end = Math.Min(at + len, content.Length);
        var window = content.Span[at..end];
        ...
        if (headerMode)
        {
            ...
            foreach (var line in AnnotateHeader(vfsPath, content))
                Console.WriteLine($"  {line}");
        }
    ...
    public static int Coverage(string[] args, string repoRoot)
    {
        ...
        foreach (var e in FormatRegistry.All)
        {
            total++;
            var dec = e.Decode is not null ? "yes" : "—";
            var conv = e.Convert == ConvertKind.None ? "—" : e.ConvertOutExt.TrimStart('.');
        ...
        var formatsDir = Path.Combine(repoRoot, "Docs", "RE", "formats");
        if (Directory.Exists(formatsDir))
        {
            ...
            foreach (var md in Directory.EnumerateFiles(formatsDir, "*.md").OrderBy(p => p, StringComparer.Ordinal))
            {
                var name = Path.GetFileName(md);
                var cited = citedSpecs.Contains(name);
    ...
    private static Skeleton? ResolveBnd(MappedVfsArchive archive, uint idB)
    {
        ...
        try
        {
            return BndParser.Parse(archive.GetFileContent(bndPath));
        }
        catch
        {
            return null;
    ...
    private static bool GuardOutputPath(string outPath, bool isDirectory, out string reason)
    {
        ...
        if (repoRoot is not null)
        {
            ...
            var candidate = isDirectory ? full : Path.GetDirectoryName(full) ?? full;
    ...
    private static IReadOnlyList<string> AnnotateHeader(string vfsPath, ReadOnlyMemory<byte> content)
    {
        ...
        if (entry.Decode is not null)
            try
            {
                lines.AddRange(entry.Decode(vfsPath, content));
            }
            catch (Exception ex)
            {
                lines.Add($"(decode hint unavailable: {ex.Message})");
            }
        else
