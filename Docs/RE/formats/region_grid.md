...
# 2026-06-24 re-verification note: regiontable record field layout REFINED — the leading 40-byte name field is
...
| Offset | Size | Type | Field | Notes | Confidence |
|-------:|-----:|------|-------|-------|------------|
| +0x00 | 32 | char[32] | `zoneName` | NUL-terminated CP949 zone display-name string (the **minimap sub-zone caption**). Three minimap/HUD label sites read the record base pointer directly as a C-string starting at offset +0. The name occupies the first 32 bytes (0x20) of the record; the NUL terminator falls within this buffer. | HIGH (read as a `char*` by 3 UI sites; width CONFIRMED by sample — name ends well before +0x20 in every decoded record; float coordinate bytes at +0x20/+0x24 are distinct from name continuation) |
| +0x20 | 4 | f32 | `labelX` | World-space X coordinate of the sub-zone label / marker (used by the minimap to place the zone caption). Not read by any gating path; role inferred from data — values fall within the map's `originX … originX + width×256` extent. | MEDIUM-HIGH (byte-clear float; role inferred from sample data, not a traced read-site) |
| +0x24 | 4 | f32 | `labelZ` | World-space Z coordinate of the sub-zone label / marker (same source/confidence as `labelX`). | MEDIUM-HIGH (byte-clear float; role inferred from sample data) |
| +0x28 | 4 | i32 | `zoneType` | Zone-type enum (see below). The **only numeric field any region-gating path reads**. | CONFIRMED |
| +0x2C | 4 | (opaque) | `_tail` | Trailing dword; **no reader found** in any path. | UNVERIFIED |

- **Record count:** fixed 32. **Record stride:** **48 bytes — CONFIRMED (RE-AFFIRMED).** Record layout: `char[32] zoneName` + `f32 labelX` + `f32 labelZ` + `i32 zoneType` + `i32 _tail` = `32 + 4 + 4 + 4 + 4 = 48` bytes. **On-disk file size:** `1,664` bytes = `1,536` record data + `128`-byte unread trailer (CONFIRMED). **Index source:** the region-id byte from the grid (Layout A) or the server-pushed active region id.
...
6. **`zoneName` exact byte width / encoding** — RESOLVED. The name buffer occupies the first **32 bytes (0x20)** of the record; bytes at `+0x20` and `+0x24` are `f32 labelX` / `f32 labelZ` (float coordinates matching the map's world extent), not name continuation. The former `48 − 4 − 4 = 40`-byte reading is superseded: the correct layout is `char[32] zoneName` + `f32 labelX` + `f32 labelZ` + `i32 zoneType` + `i32 _tail` = 48 bytes. Encoding: CP949 throughout.
