...
| 1 | Account / CharacterMgmt | C2S **and** S2C | Mixed family. S2C minors 16/17/19/20 are server-command notifications (billing / letter). C2S minors 0/6/7/9/13/14 are the character-management request builders (logout / create / select / enter / rename / slot-move). The secure 1/4 auth-reply (credential carrier) is also emitted under major 1. |
...
| `0x10000` | CmsgLogout | C2S | 0 | packets/cmsg_logout.yaml | draft | (1:0) Logout / quit-from-in-game request. Header-only (zero payload); the only char-mgmt builder that does not set the in-flight latch. Single emitter (quit-from-in-game scene path). Layout capture-unverified. |
...
| `0x10006` | CmsgCreateCharacter | C2S | 52 | packets/cmsg_char_create.yaml | draft | (1:6) **Character-CREATE only** (collision RESOLVED). The single emitter is the create-confirm action of the character-select window; an embedded 52-byte appearance/creation record (CP949 name at offset 0 + class/face/4 stats/appearance). The login credential is NOT here -- it is sub-opcode 0x2B on the secure 1/4 frame (see 0x10004). Server replies S2C 3/6/3/23 create result. Intra-payload field offsets capture-unverified. |
| `0x10007` | CmsgSelectCharacter | C2S | 2 | packets/cmsg_char_select.yaml | draft | (1:7) Slot select / ack on the char-select screen: slot index byte + a second byte (select-confirm/play flag = 1 on the play path; 0 on the create-slot ack path). Two emitters in the select-window action dispatcher. No inbound reply attributed. Field internals capture-unverified. |
| `0x10009` | CmsgEnterGameRequest | C2S | 40 | packets/cmsg_char_enter.yaml | draft | (1:9) Enter-world request: slot index @0 + 33-byte client-version blob + a derived version-check u32. Single emitter (enter-selected-character helper). Server replies 3/5 EnterGameAck. Field internals capture-unverified. |
| `0x1000d` | CmsgRenameCharacter | C2S | 18 | packets/cmsg_char_rename.yaml | draft | (1:13) Rename-character request: slot index byte @0 + 17-byte CP949 new-name buffer. Server replies 3/6 RenameCharResult (and a 3/4 subtype-1 refresh). Field internals capture-unverified. |
| `0x1000e` | CmsgMoveCharacter | C2S | 1 | packets/cmsg_char_move.yaml | draft | (1:14) Slot-MOVE / relocate request: 1-byte target slot index (the workflow-spine dossier re-attributes 1/14 from delete to slot-move; gated by slot<=4 and the in-flight latch). Two emitters in the select-window dispatcher. NOTE: prior catalog labelled 1/14 'delete'; canonical-name reconciliation owned by Tier-1. Field internals capture-unverified. |
