...
- FOLLOW-UP flagged (non-blocking; contracts/topology unaffected): `NetClient_SendProxyWorkerThread` (0x5E4525) is a DISTINCT `_endthreadex` thread firing the `1/2` keepalive + `2/112` move-heartbeat on gates 0x190/0xC8, alongside the already-named `NetClient_KeepaliveTimerThread` (the `(2,10000)` sender). So `network_dispatch.md §4.3`'s "the network-client spawns 2 threads" may UNDERCOUNT — there appears to be a send-proxy timer thread distinct from the keepalive-timer. Reconcile §4.3's NetClient thread roster in a future pass.
