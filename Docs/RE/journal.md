...
## 2026-06-20 — CYCLE 4: Netcode Deep-Cartography (DTO-grade) — contracts, field-tables, call-graphs & IDB legibility — Tier-1 + re-orchestrator waves + ida-toolsmith
- binary: doida.exe @ 263bd994 (SHA confirmed full sha256 263bd994c927c20a...; IDB module doida.exe, 25792 funcs, MCP ?ext=dbg UP). **STATIC ONLY — no debugger, no captures.**
- mandate (maintainer): a very large EXCLUSIVELY-static-IDA campaign on the entire Networking/Netcode — Client/Server contracts as DTOs (Req/Resp), Dispatcher Major/Minor + Handlers, precise naming, least doubt possible; 5 majors (1/2/3 smaller first, 4/5 = game/world server, bigger); Fire&Forget vs request/response; no captures, no debugger. Reframed: extend+verify CYCLE 3's skeleton to zero-doubt DTO-grade — every (major,minor) re-confronted to the binary then deepened to a full field-table + contract class + downstream call-graph + canonical name; promoted to specs + the IDB made fully legible.
- tool: IDA Pro 9.3 via MCP (mcp__ida__*) READONLY recovery across 6 research waves (~38 lanes) via re-orchestrator Tier-2 fan-out (re-protocol/function/struct/crypto analysts); promotion = firewall bridge (re-promote, rewrite-not-copy; general-purpose workers as spec-author absent from registry); IDB legibility by ida-toolsmith.
- scope: the primary game connection — handshake 0/0 + the 5 majors (1 Account/Billing, 2 GameAction C2S, 3 CharMgmt S2C, 4 Response S2C, 5 Push S2C). OUT: the separate lobby/login socket; C# code; crypto/LZ4 algorithms; wire VALUE semantics.
- waves (dirty, _dirty/netcode_deep/): W0 spine re-confrontation (6 lanes); W1 majors 0/1/3 (5); W2 major-2 C2S 96 builders (6); W3 major-4 Response 100 slots (10); W4 major-5 Push 65 (7); W5 body-structs + wire-vs-runtime reconcile + completeness critic (3).
- coverage: 100% DTO at the STRUCTURE level — handshake + major-1 + major-2 (96 builders / 95 opcodes set-equal to the send-census) + major-3 (11) + major-4 (100 slots + 4/500/4/50000) + major-5 (65). Final contract-class tally: Fire&Forget≈22 / Request→Response≈24 / async-pending≈68 / pure-Push≈62 / reactive 4 (1/4, 2/65, 2/146, 5/146) / specials 2.
- promoted to clean specs (rewrite-not-copy; 8 files): network_dispatch.md (4 spine corrections + 105 builders / 104 unique opcodes), opcodes.md (renames + sizes + major-2 coverage CLOSED at 95/95), handlers.md (per-handler field-map + call-graph + latch census), net_contracts.md (MASTER Req↔Resp DTO + 42-item capture/debugger-pending appendix, written LAST as the barrier), packets/*.yaml (14 created / 33 updated), structs/{net_packet_bodies, spawn_descriptor, net_client}.md.
- DIFFs the binary revealed / corrections vs prior specs: (1) 3/23 = SmsgCharStatusBytesByName (28B, keys by NAME) — refutes names.yaml "SmsgCharCreateResult"; (2) 1/7 = CmsgSelectCharacterSlot [u8 slot][u8 mode] — refutes the CYCLE-3 "delete-mode" (no major-1 char-delete opcode exists); 1/14 = CmsgMoveCharacterSlot; (3) 4/81 = SmsgActionErrorResult (generic) — the guild-diplomacy verdict rides 4/61 SmsgGuildStateChangeResult (52B); 2/81→4/61 (drop 4/81=SmsgGuildDiplomacyResult); (4) 4/48 = exactly 236B (12+8×28) — the prior "12-byte overrun" REFUTED; (5) 2/153 = CmsgProductConfirm (one 4-byte builder, 60s cooldown) — CmsgPetSummon REFUTED, the opcode-id collision RESOLVED; (6) 2/151 product-confirm — selector 0→3/8, selector 200→4/113/114/115 (the "1/20 reply" tie refuted); (7) spawn_descriptor SD+0x3C/+0x40 = ONE 64-bit HP qword (the +0x40 "current_mp" was the HP-high dword); (8) 5/53 body +0x14 = HP-high of a 64-bit HP qword, not MP; (9) 5/33 hotbar = 4-byte value table + parallel word points table (not "8 bytes at 8·slot"); (10) 5/68 quest list = 10 records (not 20); (11) 5/7 chat length-prefix EXCLUDES the NUL; 2/7 (exclude-NUL) vs 2/83 (include-NUL); (12) 2/52 = TWO builders (reactive short-form fixed-32 + player full-form variable, SkillSlot 0xFF = basic melee); (13) 2/80 = discriminated union, not an opaque 48B blob; (14) network_dispatch store counts Response 102 / Push 65, ONE shared event factory (type-15/100 vs /102, not two tags); 105 distinct builders / 104 unique opcodes (2/52 collision). 5/73 = SmsgQuestComplete re-confirmed (names.yaml was already correct; the IDB autoname was stale → fixed).
- IDB legibility (ida-toolsmith, applied + saved): the netcode was already ~99% named (338 fns) from prior campaigns, so this was a correction+comment+type pass — 5 renames (5/73→SmsgQuestComplete, 3/23→SmsgCharStatusBytesByName, 4/56→SmsgSceneEnterSpawnSnapshot, 4/71→SmsgRelationPartyPanelSnapshot, 4/74→SmsgStallListRefill) + 101 neutral comments + 4 struct types (SkillActionTargetRecord / BuffEntry / TrackedPanelSlotRecord / CmsgGuildRequestBody) + the NetConnStateCode enum NEUTRALIZED to value-only members ({201/202/203/232} meanings are capture-pending — the worker's invented English member names were rejected per the Ground-Truth Doctrine); SHA-pinned 263bd994, idempotent, idb_save OK; staged to _dirty/netcode_deep/applied/cycle4_idb_names.md.
- names.yaml (Tier-1 sync, this cycle): SmsgResponseSlot56/71/74 → SmsgSceneEnterSpawnSnapshot / SmsgRelationPartyPanelSnapshot / SmsgStallListRefill (both the address-keyed and opcode-keyed sections); 0x30017 (3/23) SmsgCharCreateResult → SmsgCharStatusBytesByName; 5/73 already correct (no change).
- firewall: clean-room audit PASS (Tier-1 independent scan + plan-reviewer audit) — zero virtual addresses, zero Hex-Rays artifacts across the 8 promoted specs + packets/ + structs/; all addresses confined to _dirty/ (gitignored). plan-reviewer gate verdict = PASS (zero-gap routing surface, over-claim audit honest, banners present, master synthesis complete); the only flagged nits (2 stale yaml links, 1 stale coverage note, 1 cosmetic role sentence) all fixed.
- completeness: 100% DTO-covered, ZERO structural gaps, ZERO unresolved cross-wave contradictions. The single uniform residual = wire-byte VALUE semantics — a consolidated 42-item [capture/debugger-pending] register (R-01..R-42) in net_contracts.md Appendix C, the honest worklist for a future ?ext=dbg debugger cycle.
- notes: capture_verified: false (static-only; all wire VALUE semantics capture/debugger-pending — never fabricated). OPEN arbitration carried: C1 — 1/2 CmsgLobbyPing routes through the SAME NetClient send convergence as char-mgmt on this build (tension with the separate-lobby-socket doctrine; recorded for a future lobby-socket lane; surfaced to maintainer); 2/75 roster-assemble interior opaque (debugger-pending). Journalled as CYCLE 4. Uncommitted on branch major-campaign.
- addendum (2026-06-20, maintainer interop note + client-side static confirm): the SERVER leaves Nagle's algorithm ENABLED (no `TCP_NODELAY`) on the game connection — most pronounced for the **World Server** (major-4 Response / major-5 Push) — so it **coalesces several application frames into one TCP segment** (and may split one frame across segments). The CLIENT is symmetric: its sole `setsockopt` sets `SO_RCVBUF` at `SOL_SOCKET`, and there is **no `TCP_NODELAY` call anywhere** (`[confirmed]` static), so the client send path is Nagle-subject too. This is the WHY behind the length-prefixed frame-reassembly loop — implementers must frame on the header `size`, never "one recv = one message". Documented: `network_dispatch.md §4.4a` (full) + `net_contracts.md §1.1` (carried-for-implementers). Server-side Nagle = maintainer-observed runtime fact; client-side = static-confirmed. firewall: neutral prose, no addresses.

## 2026-06-20 — CYCLE 5: Network connection topology + login→game/world handoff — Tier-1 + re-orchestrator
- binary: doida.exe @ 263bd994 (STATIC-IDA only; no debugger, no captures).
- mandate: continue the necessary netcode work after CYCLE 4 on a NON-contested surface (a concurrent session owns the layer-02 C# port + `crypto.md`/`login_flow.md`/`lobby.yaml`).
- scope: map the client's socket TOPOLOGY — how many sockets, which server each serves, which opcode majors ride which socket, the connection lifecycle vs the 8-state scene FSM, the login→game/world server-address handoff; and RESOLVE C1.
- tool: re-orchestrator 4-lane READONLY wave (socket census / C1 routing / lifecycle-vs-scene / address handoff); dirty findings in `_dirty/netcode_deep/topology/*.md`.
- promoted to clean specs: NEW `Docs/RE/specs/connection_topology.md` (master) + a cross-ref in `network_dispatch.md §4`. Did NOT touch the concurrent-owned `crypto.md`/`login_flow.md`/`lobby.yaml`.
- findings: (1) 3 socket subsystems but exactly ONE persistent opcode connection (A) — ALL majors 0-5 (login + char-mgmt + game + the World Server Response/Push) ride the SAME socket A; there is NO separate world socket and NO login→game reconnect, only an ADDRESS handoff. (2) B = login/lobby blocking queries (`inet_addr`, header-less, port 10000 / 10000+server_id; the channel-endpoint query returns a 30-byte `"<host> <port>"` token that becomes A's target via DNS). (3) C = XTrap anti-cheat relay (binary-literal `211.115.86.66:2424`, `XL_XF_V1`). (4) **C1 RESOLVED**: `1/2 CmsgLobbyPing` rides socket A (functionally a game keepalive), NOT a lobby socket — the lobby threads never call the 1/2 builder. (5) lifecycle: socket A connects ONCE at the login join handoff (~substate 40→41) after the lobby queries stage the endpoint token; enter-world rides the open socket; disconnect only on error/exit (leave-world does NOT close A). (6) game host is unknown locally → sole source = the channel-endpoint token (no opcode redirect); lobby host = `ip.txt` → `list.dat`/registry → fallback `211.196.150.4`.
- CORRECTION vs CYCLE-4 notes: a "lobby socket helper" cluster (socket+inet_addr+htons ctor, blocking connect-with-retry, getsockname) was actually the **XTrap** relay, not the server-list path; the real server-list path is the `ip.txt` probe.
- firewall: PASS (`connection_topology.md` + the `network_dispatch.md` cross-ref scanned clean; the embedded endpoint constants are interop DATA facts, allowed; all addresses/pseudo-C confined to gitignored `_dirty/`). No IDB mutation, no `journal`/`names.yaml` touched by workers.
- notes: capture_verified: false. Runtime host/port VALUES (lobby + game) beyond binary literals and the conn-state code meanings 201/202/203/232 stay capture/debugger-pending. Committed targeted on branch major-campaign (`474b846`).
- IDB legibility (ida-toolsmith, applied + saved): **18 renames + 24 neutral comments** on the topology routines (`sub_` −18); `NetConn_{CreateSocket, ConnectDnsAndSelect, ConnectAndStartIoThread, SetRecvBuf, WSARecvRearm}`, `NetClient_{HandleConnectCommand, StoreHostPort, SendProxyWorkerThread}`, `Login_{LobbyConnect, FetchChannelEndpointThread, FetchServerRosterThread, StartNetworkAndPostConnect, ListDatRecordSelector}`, `XTrap_{Socket_ctor, SocketSetup, ConnectWithRetry, BuildXlXfV1Packet}`; plus the C1 correction `Cmsg_LobbyPing_Send → Cmsg_GameConnKeepalive_Send` (1/2 rides socket A, not a lobby socket). SHA-pinned 263bd994, idempotent, idb_save OK; staged to `_dirty/netcode_deep/applied/cycle5_topology_idb_names.md`. names.yaml unchanged (these were IDB-only `sub_` names absent from the committed glossary; bulk sync deferred per norm).
- §4.3/§4.5 THREAD-ROSTER reconciliation — **RESOLVED this session** (CYCLE 5): decompile-confirmed that `NetClient_SpawnWorkerThreads` (start-engine) spawns exactly TWO slots (recv-consumer + keepalive-timer), and `NetClient_SendProxyWorkerThread` (0x5E4525) is a genuine **FOURTH** netcode thread spawned by a separate routine (`sub_5E432C`) — a ~10 ms loop that fires the **`1/2`** game-connection keepalive (idle gate, 400 ms latency budget) and the **`2/112`** move-heartbeat (world-enter gate, 200 ms budget); the `(2,10000)` cached frame stays on thread #2. Corrected `network_dispatch.md §4.3` (THREE→FOUR threads, new thread-#4 row) and §4.5 (keepalive/heartbeat = **three periodic sends on two threads**: `(2,10000)`/thread-2 · `1/2` + `2/112`/thread-4; the `2/112` is a periodic heartbeat, NOT a one-off toggle). Contracts/opcodes unchanged; firewall clean.

## 2026-06-20 — LIVE INTEGRATION: C# + Godot networking validated 1:1 vs the LIVE replica (login→char-select→enter-world) + x32dbg ground-truth — Tier-1 + port-orchestrator + x32dbg
- oracle (NEW this cycle — dual/live): the LIVE replica server (lobby `211.196.150.4:10000`; game channel `211.196.150.4:114xx`) for wire VALUE semantics, AND the x32dbg live debugger on `doida.exe` (maintainer-launched; base 0x400000 no-ASLR → IDA addrs direct; SHA 263bd994) for runtime ground-truth. This is the FIRST cycle to connect the clean-room C# client to the live server and confirm the wire end-to-end — complements the prior STATIC-only cartography cycles (CYCLE 3/4/5).
- mandate (maintainer): implement ALL client networking cleanly and faithfully 1:1, pulling from IDA + confirming live, in C# AND Godot, to make the client "exploitable" (a live playable slice login→world). NO xUnit this cycle (maintainer directive) — verification via build + headless + the live loop.
- milestones (all 1:1 live): M2 login ✅ · M3 char-select ✅ · M4 enter-world ✅. The Godot client does the full chain AUTONOMOUSLY: Init→Login→lobby→connect→`0/0`→`1/4`→`3/4` roster (3 chars)→auto-select slot 0→`1/9`→**`4/1` SmsgGameStateTick world snapshot (9100B)** (creds via env; SessionToken = MD5(doida.exe) = `a1437026…`).
- THREE live-caught fidelity bugs FIXED — each a CODE-vs-SPEC divergence where the SPEC was already right (validates the clean-room doctrine: spec = truth, code measured against it):
  1. Lobby channel-endpoint — client required ≥30B, but 30 is the COPY-CAP not a minimum; the live server returns a 23B `"<host> <port>"` token → relaxed to `len==0→throw` + parse `min(len,30)` NUL-terminated. (`LobbyClient` — committed earlier this session.)
  2. `1/4` login whitening — `XorWhitenDwords` floored the span (`length & ~3`), leaving the trailing partial dword un-whitened → server rejected with `3/100` code 10 (password-INDEPENDENT) → fixed to whiten ALL bytes incl. the trailing partial dword. (`HandshakeWhitening` — committed earlier this session.)
  3. TCP `NoDelay` — `TcpTransport` set `Socket.NoDelay = true` (Nagle OFF) → the server REJECTED the byte-identical `1/9` enter (`3/100` code 8/23, no world data); with `NoDelay = false` (Nagle ON) the SAME `1/9` is ACCEPTED and the `4/1` world snapshot arrives. `network_dispatch.md §4.4a` ALREADY stated `[confirmed]` the client never sets `TCP_NODELAY` (sole `setsockopt` = `SO_RCVBUF`) → the CODE diverged, NOT the spec → NO spec change. (`TcpTransport` — THIS commit; env `MH_TCP_NODELAY=1` forces true for diagnostics.)
- x32dbg ground-truth (dirty, gitignored `_dirty/dbg/`): a conditional BP on the send-convergence `Net_SendPacket` (`0x618f35`, condition skips the `1/4`) during a real-client enter proved (a) the enter C2S sequence is exactly `login(1/4) → 1/9` (no `1/7`, no keepalive between), and (b) the `1/9` payload is BYTE-IDENTICAL to the C# client's: `[slot=0][token "a1437026…"+NUL][pad][version 0x529d=21149]`. ⟹ the protocol was proven correct end-to-end; the ONLY enter difference was the TCP NoDelay flag.
- Godot live-enter wiring (THIS commit, layers 04+05): SessionToken resolver in `ClientContext` (env `MH_SESSION_TOKEN` → `clientdata/session_token.txt` → dev-fallback MD5; ASCII into `byte[33]`, NUL-terminated); `LiveLoginAutoload` auto-selects slot 0 + fires the `1/9` once after the roster poll; `SceneStateMachine` `EnteringWorld` latch (the enter-ack arms it; `AdvanceLoadScene` routes Load→InGame; the OFFLINE path is unchanged).
- spec promotions (committed earlier this session, not this commit): `lobby.yaml` (endpoint copy-cap), `login_flow.md` step 7 (3/4 = char roster, NOT SceneEntityUpdate), `crypto.md` §6.2.2/§6.4 (whitening span), `net_contracts.md` §2.1. M4 required NO spec change.
- gates: nuke `dotnet build MartialHeroes.slnx` = **0 Erreur(s)** (4 NU1903 SQLite warnings pre-existing), 14 projects incl. the Godot client; firewall PASS (zero decompiler artifacts in the diff; `// spec:` citations present); DAG clean (no `using Godot;` in the layer-04 `SceneStateMachine`); NO xUnit (maintainer directive).
- KNOWN-REMAINING (deferred to a dedicated follow-up cycle; NOT blockers): the world does not yet RENDER from the live `4/1` — the scene stays at "Load" (the post-login `3/5` fires Login→Load prematurely; the `4/1` must drive Load/select→InGame + local-player spawn). A FRONT-END scene-flow task (layer 04 `SceneStateMachine` + 05 scenes), distinct from the networking (which is complete). Also deferred: `SO_RCVBUF` socket option (fidelity; enter works without it); the prior cartography cycles' names.yaml hand-merge (this live cycle added NO IDB renames — x32dbg was runtime confirmation only).
- notes: NetSmoke diagnostic harness REMOVED (throwaway one-off; recoverable from git history; the live loop now runs via the Godot `LiveLoginAutoload`). `.mcp.json` registers the x32dbg MCP (the debugger that cracked M4). capture_verified via the LIVE server (not a `.pcapng`). Committed targeted on branch major-campaign.
