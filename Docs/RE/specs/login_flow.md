...
verification: confirmed (lobby host-resolution order, registry keys, the 0/0->1/4 handshake
  ...
  CYCLE 14 re-anchor (f61f66a9): 8 facts re-confirmed SAME, 0 corrected.
ida_reverified: 2026-06-27   # CYCLE 14 static-only re-anchor pass (f61f66a9): login cluster — lobby host-resolution chain (ip.txt/list.dat/211.196.150.4 + inet_addr), server-list 8-byte record decode and commit gate, game.ver index-5 equality gate, enter-game request (1/9) 40-byte body and slot-index field, slot caching (880-byte descriptor + 96-byte stats), empty-slot sentinel "@BLANK@", login credential blob (1/4 pre-image sub-opcode 0x2B + u32-LE NUL-inclusive length prefixes), RSA key-exchange shape — all 8 facts unchanged against the new build. Crypto-session lane (AuthSession_BuildLoginPacket43) not re-traced this static pass; prior runtime-confirmed facts remain the basis.
