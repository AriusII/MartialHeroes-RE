---
verification: confirmed — CYCLE 14 re-anchor (f61f66a9): 1 fact re-confirmed SAME (GUPanel/GUWindow slot-10 active-child getter; Concurrency mislabel noted in guwindow.md §3.1)
...
| Offset | Size | Field | Notes |
|-------:|-----:|-------|-------|
| +0x8B | 1 | `caret_active` / focused | caret-blink runs only when set; Tab focus-swap toggles it between fields. |
| +0xA4 | 4 | `charset_mask` (ctor init 1) | **Input filter / charset + password-mask byte.** Bit **0x80** activates `'*'`-per-char password masking in the draw path (`(+0xA4 & 0x80) == 0x80`); the low bits are a charset/validation selector. The login `BuildScene` writes **6** (ID textbox — bit 0x80 clear, unmasked) and **0x81** (PW textbox — bit 0x80 set, masked). **This is NOT a max-length field.** See `Docs/RE/specs/frontend_layout_tables.md §0.11 / §2.7`. **(Corrected: the earlier `max_length` label is refuted by the binary draw path — CODE-CONFIRMED IDB SHA 263bd994.)** |
...
| +0xD0 | 4 | `ime_conversion_mode` (ctor init 10) | **IME conversion-mode word** — not a style/mask field. The login `BuildScene` writes **16** for the ID textbox and **12** for the PW textbox. The password-mask bit is in **+0xA4 bit 0x80**, not here. **(Corrected: the earlier `mode_style_word` label and the incorrect claim that the mask bit lives here are refuted — CODE-CONFIRMED IDB SHA 263bd994.)** |
...
> **Field-assignment correction (CODE-CONFIRMED — IDB SHA 263bd994).**
> The GUTextbox draw path checks `(+0xA4 & 0x80) == 0x80` to activate `'*'`-per-char password
> masking. `+0xA4` is therefore the **charset/filter + mask byte**, not a max-length cap, and +0xD0
> is the **IME conversion-mode word** (ctor init 10; login form writes 16/12), not a style-flags word.
> An earlier revision of this table had `max_length` at +0xA4 and placed the password-mask bit in
> `+0xD0`; both assignments are **refuted by the binary draw path** and are superseded here. The
> canonical mapping is also confirmed by `Docs/RE/specs/frontend_layout_tables.md §0.11 / §2.7`.
>
> **Open item — true max-length field:** since +0xA4 (6/0x81) is the filter/mask byte and +0xD0
> (16/12) is the IME conversion-mode word, neither carries the per-keystroke editable-character cap.
> The cap field was not pinned in this audit pass and is a genuine open item (see §12 residuals).
...
- **Password mode** (enabled when **bit 0x80 of the `charset_mask` byte at +0xA4** is set — see
  §1.5a; the draw path checks `(+0xA4 & 0x80) == 0x80`): draws one `'*'` glyph per character,
...
18. **GUTextbox editable max-length field location.** The earlier assignment of `+0xA4 = max_length`
    is refuted (§1.5a): +0xA4 is the charset/filter + mask byte and +0xD0 is the IME conversion-mode
    word. The per-keystroke editable-character cap lives at an offset not yet pinned in this pass;
    this is a genuine open item requiring a further static pass or a debugger confirmation.
