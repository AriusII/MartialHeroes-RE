...
| Response / push tables | **CONFIRMED** — two abutting 154-slot (616-byte) function-pointer tables at +0x1378 and +0x15E0, each pre-filled with a single no-op handler then overwritten by an installer. |
...
| +0x1378 | 1246 | 616 | ptr[154] | `response_handler_table` | CONFIRMED | 154 function-pointer slots for **major-4** server-response (S2C) handlers. Pre-filled with a single no-op handler, then overwritten by the response-table installer. The dispatcher routes major-4 by reading slot `(minor)` of this table. |
| +0x15E0 | 1400 | 616 | ptr[154] | `push_handler_table` | CONFIRMED | 154 function-pointer slots for **major-5** server-push (S2C push) handlers. Same initialisation pattern (no-op fill → push-table installer). Abuts the response table (1400 − 1246 = 154). Ends at the object tail. |
...
- **Two dispatch tables, 154 slots each.** The constructor pre-fills both tables with one inert
  no-op handler (a slot that simply returns success), then runs two installer routines that
  overwrite the real handler slots. Major-4 packets route through `response_handler_table`; major-5
  packets route through `push_handler_table`. Majors 1 and 3 are **inline switches**, not table
  driven (cross-ref `specs/network_dispatch.md`).
...
- **Initial handshake staging.** The constructor stages the initial client handshake packet
  (major 2, minor 10000) before any authentication flow begins (cross-ref `structs/net_client.md`
  keepalive note and `specs/login_flow.md`).
