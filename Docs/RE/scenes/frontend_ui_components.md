---
verification: written 2026-06-21 against the doida.exe binary (build 263bd994, static IDA). This is a
  CROSS-SCENE INDEX, not a fresh recovery — it ties together the per-scene dossiers
...
|---|---|---|---|---|---|
| **1** | **Login** — `LoginWindow` | `LoginWindow : CommonLoginWindow : GUWindow : GUPanel : GUComponent : EventHandler` | **server-list panel** (header img, enter btn, login-form sub-panel, quit/help); **login-form panel** (ID/PW label imgs, account `GUTextbox`, password `GUTextbox` masked, save-ID `GUCheckBox`, OK `GUButton`); **EULA/agreement panel** (22 `GULabel` rows 4001..4022 + scroll up/down/thumb buttons + track img); **server-grid panel** (per-row name/flag/select/pop/ping, status icons, 10 name-strip buttons); **confirm panels A/B** (`GULabel` + OK); **second-password PIN modal** (`LoginSecondPassword` — scrambled keypad up to 100 `GUButton`, OK/Clear/Cancel, embedded `ExitPanel`); **ExitPanel** (quit confirm); **ErrorPanel** (notice host). ~73 widgets total. | Yes (action-ids 102–124, plate 400/401, PIN 0–99/11/12/13) | [scenes/login.md](login.md) |
| **2** | **Loading** — `LoadHandler` + `LoadingScreen` | composite `LoadHandler` ("Loader") + `LoadingScreen` visual phase, embedding a `Diamond::GView` sub-object — **NOT** a `GUWindow` | **background quad** (immediate-mode textured quad, random of `loading.dds`/`loading06.dds`/`loading08.dds`); **progress-gauge quad** (immediate-mode textured quad, width tracks VFS progress); **embedded scene-view** (`Diamond::GView`); **looping SFX** (id 920100100). **No `GUButton`/`GULabel`/`GUPanel`/`GUTextbox`/modal — no action-ids.** | No (no interactive widgets, no action-ids) | [scenes/load.md](load.md) |
...
`maxLength` and an IME-mode field, and blinks a caret (~500 ms) while focused. Login's password box
sets this bit (IME mode 12, maxlen 129, masked); the account box does not (IME mode 16, maxlen 6).
Opening and Loading own no textbox.
...
| World content | Streamed terrain (3×3 cold-start), shared `EnvironmentLightScene` light, env pinned to **14:30** (keyframe 29), ambient XEffect 380003000, **preview actors** (5 lineup @3× / 1 zoom @2×) spawned via the same actor factory as the live world | Full live world via a `GScene` root + `TerrainManager` + `ActorManager`, a `GSwitch` of 5 draw layers (terrain / buildings+static / world-objects / actors / effects), sky + lens-flare prep |
