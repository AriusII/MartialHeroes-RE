...
public static class SkyBoxParser
{
    ...
    public static SkyBoxData Parse(ReadOnlySpan<byte> span)
    {
        ...
        var textureCount = BinaryPrimitives.ReadUInt32LittleEndian(span[cursor..]);
        cursor += 4;
        ...
        var nameBlockSize = n * TextureNameStride;
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var nameSpan = span.Slice(cursor, TextureNameStride);
            var nullPos = nameSpan.IndexOf((byte)0);
            var nameLen = nullPos >= 0 ? nullPos : TextureNameStride;
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var vertexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[cursor..]);
            cursor += 4;
            ...
            var vc = (int)vertexCount;
            var vertexBlockBytes = vc * VertexStride;
            ...
            for (var v = 0; v < vc; v++)
            {
                var vBase = cursor + v * VertexStride;

                // position (x, y, z) — f32[3] @ sub-offset 0x00.
                // spec: Docs/RE/formats/sky.md §A.4 — position f32[3] @ sub-offset 0x00: MED
                var x = BinaryPrimitives.ReadSingleLittleEndian(span[(vBase + 0x00)..]);
                var y = BinaryPrimitives.ReadSingleLittleEndian(span[(vBase + 0x04)..]);
                var z = BinaryPrimitives.ReadSingleLittleEndian(span[(vBase + 0x08)..]);

                // uv (u, v) — f32[2] @ sub-offset 0x0C.
                // spec: Docs/RE/formats/sky.md §A.4 — uv f32[2] @ sub-offset 0x0C: MED
                var u = BinaryPrimitives.ReadSingleLittleEndian(span[(vBase + 0x0C)..]);
                var vt = BinaryPrimitives.ReadSingleLittleEndian(span[(vBase + 0x10)..]);
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var indexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[cursor..]);
            cursor += 4;
            ...
            var ic = (int)indexCount;
            var indexBlockBytes = ic * IndexWidth;
