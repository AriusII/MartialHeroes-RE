...
public static class XeffParser
{
    ...
    public static XeffData ParseXeff(ReadOnlySpan<byte> span)
    {
        ...
        var offset = XeffHeaderSize; // 0x08
        ...
        ReadOnlySpan<byte> span, ref int offset, int subIndex)
    {
        ...
        else
        {
            ...
            var hasRotation = emitterType == EmitterDirectional; // emitter_type == 2
        ...
        ReadOnlySpan<byte> span, ref int offset, uint kfIndex, int subIndex, int k)
    {
        ...
        // spec: Docs/RE/formats/effects.md §A.4.4 nine-float layout: CONFIRMED.
        var vx = BinaryPrimitives.ReadSingleLittleEndian(span[offset..]); // position 1
        var vy = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 4)..]); // position 2
        var vz = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 8)..]); // position 3
        var sx = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 12)..]); // position 4
        var sy = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 16)..]); // position 5
        var sz = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 20)..]); // position 6
        var rxd = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 24)..]); // position 7 — degrees
        var ryd = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 28)..]); // position 8 — degrees
        var rzd = BinaryPrimitives.ReadSingleLittleEndian(span[(offset + 32)..]); // position 9 — degrees
