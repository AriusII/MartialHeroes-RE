...
public static class DoStanceParser
{
    ...
    public static DoStanceTable Parse(ReadOnlyMemory<byte> data) => Parse(data.Span);
    ...
    public static DoStanceTable Parse(ReadOnlySpan<byte> span)
    {
        ...
        int stride = DoStanceRecord.Stride; // 116
        int totalCount = span.Length / stride;
        int trailingBytes = span.Length % stride;
        ...
        for (int i = 0; i < totalCount; i++)
        {
            ...
            int recordBase = i * stride;
            ReadOnlySpan<byte> rec = span.Slice(recordBase, stride);
            ...
            uint instanceKey = BinaryPrimitives.ReadUInt32LittleEndian(rec[OffInstanceKey..]);

            // groupSubIndex u32 LE @ record+0x04. SAMPLE-VERIFIED.
            // spec: Docs/RE/formats/ui_manifests.md §2.7 — "+0x04 u32 groupSubIndex".
            uint groupSubIndex = BinaryPrimitives.ReadUInt32LittleEndian(rec[OffGroupSubIndex..]);
            ...
            uint classStanceRef = BinaryPrimitives.ReadUInt32LittleEndian(rec[OffClassStanceRef..]);

            // groupId u32 LE @ record+0x10. SAMPLE-VERIFIED.
            // spec: Docs/RE/formats/ui_manifests.md §2.7 — "+0x10 u32 groupId".
            uint groupId = BinaryPrimitives.ReadUInt32LittleEndian(rec[OffGroupId..]);
            ...
            ushort secondarySpriteX = BinaryPrimitives.ReadUInt16LittleEndian(rec[OffSecondarySpriteX..]);
            ...
            ushort secondarySpriteY = BinaryPrimitives.ReadUInt16LittleEndian(rec[OffSecondarySpriteY..]);
