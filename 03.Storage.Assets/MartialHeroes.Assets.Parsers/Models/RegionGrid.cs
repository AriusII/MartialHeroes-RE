...
/// NOT parsed here.
/// spec: Docs/RE/formats/region_grid.md §Two layouts — ".tol (authoring / tool-side — not read by the shipped client)".
/// </para>
/// <para>
...
/// </code>
/// spec: Docs/RE/formats/region_grid.md §Runtime use — "index = (X − originX) / 256 + (Z − originZ) / 256 × width": HIGH (parser).
/// </para>
...
public sealed class RegionGrid
{
    ...
    /// spec: Docs/RE/formats/region_grid.md §Layout A — "regionIdGrid u8[] @ 0x08, width × height bytes, row-major": HIGH (parser).
    ...
    public byte GetRegionId(int worldX, int worldZ)
    {
        ...
        int col = (worldX - OriginX) / CellWorldSize; // spec: Docs/RE/formats/region_grid.md §Runtime use
        int row = (worldZ - OriginZ) / CellWorldSize; // spec: Docs/RE/formats/region_grid.md §Runtime use
