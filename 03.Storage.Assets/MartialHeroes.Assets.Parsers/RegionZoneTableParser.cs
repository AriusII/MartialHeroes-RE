...
/// file through the 32-byte-stride sub-zone-label lens (spec: <c>Docs/RE/formats/misc_data.md
/// §7.2</c>). The two parsers expose different views of the same file for different consumers:
...
public static class RegionZoneTableParser
{
    ...
    public static RegionZoneRecord[] Parse(ReadOnlyMemory<byte> data)
    {
        ...
        for (int regionId = 0; regionId < RecordCount; regionId++)
        {
            int recBase = regionId * RecordStride;
            ...
            ReadOnlyMemory<byte> opaqueLeading = data.Slice(recBase, OpaqueLeadingSize);

            // Zone-type u32le at +40 — the only field consumed by region-gating logic.
            // spec: Docs/RE/specs/world_systems.md Ch. 16 §16.2 — "+40 4 u32 zone type": CONFIRMED (encoding).
            uint zoneTypeRaw = BinaryPrimitives.ReadUInt32LittleEndian(
                span.Slice(recBase + ZoneTypeOffset, 4));

            // Opaque trailing bytes at +44..+47 (preserved but not interpreted).
            // spec: Docs/RE/specs/world_systems.md Ch. 16 §16.2 — "+44 4 (opaque) trailing bytes": UNVERIFIED.
            ReadOnlyMemory<byte> opaqueTrailing = data.Slice(recBase + OpaqueTrailingOffset, OpaqueTrailingSize);
