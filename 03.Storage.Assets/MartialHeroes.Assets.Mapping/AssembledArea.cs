...
public sealed class AssembledCell
{
    ...
    /// </summary>
    public long CellKey => MapZ + 100000L * MapX; // spec: terrain-manager.md §Key formula
...
public sealed class AssembledArea
{
    ...
    /// <summary>
    ///     Neutral NPC spawn descriptors built from <c>npc&lt;NNN&gt;.arr</c> (per-area binary #4).
    ///     Ground Y is NOT baked — re-sample per frame via <see cref="AssembledCell.SampleHeight" />.
    ///     Live mob actors arrive from the server via packet 4/4 and are not represented here.
    ///     spec: Docs/RE/formats/area_inventory.md §1A.3 — npc&lt;NNN&gt;.arr is per-area binary #4: CODE-CONFIRMED.
    ...
    ///     spec: Docs/RE/formats/area_inventory.md §5.1 — no runtime mob.arr loader; mob actors are server-driven.
    /// </summary>
...
/// <summary>
///     Neutral NPC spawn descriptor — position and facing from <c>npc&lt;NNN&gt;.arr</c>
///     (28-byte records, per-area binary #4). NO baked Y.
/// </summary>
/// <remarks>
///     spec: Docs/RE/formats/area_inventory.md §1A.3 — npc&lt;NNN&gt;.arr is per-area binary #4: CODE-CONFIRMED.
///     spec: Docs/RE/formats/npc_spawns.md §Runtime role — "Ground-Y is re-sampled from terrain
...
public readonly record struct SpawnDescriptor
{
    ...
    /// <summary>
    ///     True when this descriptor was read from <c>npc&lt;NNN&gt;.arr</c> (NPC kind).
    ///     Always <see langword="true" /> for records built by <see cref="AreaComposer" />; the field is
    ///     preserved on the model so downstream consumers (layer 04/05) can distinguish NPC vs mob actors
    ///     when mob descriptors arrive from alternate sources (e.g. server packet 4/4 adapters).
    ///     spec: Docs/RE/formats/area_inventory.md §1A.3 — npc.arr is the only .arr binary #4 that has
    ///     a runtime client loader. CODE-CONFIRMED.
    ///     spec: Docs/RE/formats/area_inventory.md §5.1 — no runtime mob.arr loader; mob actors are server-driven.
    /// </summary>
