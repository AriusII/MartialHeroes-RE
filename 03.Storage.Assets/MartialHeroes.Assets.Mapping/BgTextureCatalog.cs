...
public sealed class BgTextureCatalog
{
    ...
    private BgTextureCatalog(string?[] relPathBySlot) => _relPathBySlot = relPathBySlot;
    ...
    /// <b>runtime form</b> the original client consumes. Prefer this over <see cref="FromTxt"/>.
    /// </summary>
    ...
    public static BgTextureCatalog FromLst(ReadOnlyMemory<byte> lstBytes) =>
        FromLst(BgtextureLstParser.Parse(lstBytes));
    ...
    public static BgTextureCatalog FromLst(BgtextureLstCatalog parsed)
    {
        ...
        foreach (BgtextureLstRecord record in parsed.Records)
        {
    ...
    /// <summary>
    /// Builds the catalogue from the <c>bgtexture.txt</c> text mirror — a <b>dev / loose-tree
    /// fallback only</b>. The text mirror is absent from a real packed <c>data.vfs</c>; consumers
    /// must use <see cref="FromLst(ReadOnlyMemory{byte})"/> against a real client and fall back to
    /// this only when the <c>.lst</c> is missing.
    /// </summary>
    /// <param name="txtBytes">Raw <c>bgtexture.txt</c> file bytes (CP949, TAB-separated).</param>
    /// <remarks>
    /// spec: Docs/RE/specs/asset_pipeline.md §3 chain B — <c>bgtexture.txt</c> is the human-readable
    ///   mirror, absent at runtime; the <c>.lst</c> wins where they disagree. CONFIRMED.
    /// spec: Docs/RE/formats/bgtexture_lst.md §Text mirror — col0 = the 0-based record index, col1 =
    ///   the kind/render-mode tag, col2 = the relpath; the row count equals the <c>.lst</c>
    ///   record_count. CONFIRMED.
    /// </remarks>
    public static BgTextureCatalog FromTxt(ReadOnlyMemory<byte> txtBytes)
    {
        // The text mirror keys rows by the 0-based record index (col0), which is the same pool slot
        // as the .lst record position. Parse the CP949 / TAB rows directly so the slot index is
        // stable (the array is sized to the highest index + 1; gaps resolve to null).
        // spec: Docs/RE/formats/bgtexture_lst.md §Text mirror — TAB-separated, no header row,
        //   col0 = 0-based index, col2 = relpath (no .dds). CONFIRMED.
        Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
        string text = Encoding.GetEncoding(949).GetString(txtBytes.Span);
        ...
        foreach (string rawLine in text.Split('\n'))
        {
            ...
            if (cols.Length < 3)
                continue;
            if (!int.TryParse(cols[0].Trim(), out int index) || index < 0)
                continue;

            string rel = cols[2].Trim();
            if (rel.Length == 0)
                continue;
        ...
        foreach ((int index, string rel) in byIndex)
            slots[index] = rel;
    ...
    public string? ResolveRelativePath(int poolSlot) =>
        (uint)poolSlot < (uint)_relPathBySlot.Length ? _relPathBySlot[poolSlot] : null;
    ...
    public string? ResolveTexturePath(int poolSlot, string textureDir = TerrainTextureDir)
    {
        ...
        string? rel = ResolveRelativePath(poolSlot);
