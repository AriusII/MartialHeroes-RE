...
public static class TerrainLayerParsers
{
    ...
    private static CollisionTriangle ReadCollisionTriangle(ReadOnlySpan<byte> rec)
    {
        ...
        return new CollisionTriangle(
            BinaryPrimitives.ReadSingleLittleEndian(rec[..]), // v1_x
    ...
    ///     Universal group-array layout: u32 group_count, then group_count × [20-byte group header + VF_36 vertices + u16
    ///     indices].
    /// </summary>
    /// <param name="data">Raw file bytes from VFS.</param>
    ...
    private static Fx1Layer ParseFx1(ReadOnlySpan<byte> span, ReadOnlyMemory<byte> backing)
    {
        ...
        for (uint g = 0; g < groupCount; g++)
        {
            ...
            var vertexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[(offset + 12)..]); // +0x0C CONFIRMED
            var indexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[(offset + 16)..]); // +0x10 CONFIRMED
    ...
    ///     Universal group-array layout: u32 group_count, then group_count × [20-byte group header + VF_44 vertices + u16
    ///     indices].
    /// </summary>
    /// <remarks>
    ...
    ///     Universal group-array layout: u32 group_count, then group_count × [44-byte extended group header + VF_36 vertices +
    ///     u16 indices].
    /// </summary>
    /// <remarks>
    ...
    private static Fx3Layer ParseFx3(ReadOnlySpan<byte> span, ReadOnlyMemory<byte> backing)
    {
        ...
        for (uint g = 0; g < groupCount; g++)
        {
            ...
            var vertexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[(offset + 36)..]); // +0x24 CONFIRMED
            var indexCount = BinaryPrimitives.ReadUInt32LittleEndian(span[(offset + 40)..]); // +0x28 CONFIRMED
    ...
    ///     Universal group-array layout: u32 group_count, then group_count × [ 48-byte group header + VF_36 vertices + u16
    ///     indices ].
    ...
    ///     The correct model is the universal group-array (§1.1a): 48-byte group header with vertex_count@+0x28 and
    ///     index_count@+0x2C.
    /// </remarks>
    ...
    ///     Universal group-array layout: u32 group_count, then group_count × [ 52-byte group header + VF_32 vertices + u16
    ///     indices ].
