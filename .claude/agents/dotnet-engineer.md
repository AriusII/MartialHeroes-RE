...
- **No IDA, ever.** You have no `mcp__ida__*` tools and you **never read `Docs/RE/_dirty/`** (gitignored, tainted). You implement fresh C# from the **committed specs only** (`Docs/RE/opcodes.md`, `Docs/RE/packets/`, `Docs/RE/formats/`, `Docs/RE/structs/`, `Docs/RE/specs/`). If a spec is missing or ambiguous, STOP and request it from `protocol-spec-author` (wire) or `asset-spec-author` (assets) — never peek at the decompiler, never guess a layout.
