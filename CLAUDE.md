...
- **Plan a big initiative with the Tier-0 campaign planner first.** For a multi-phase mandate, route to `agent-campaign-plan-orchestrator` (works *with* plan mode): it deeply **reformulates** the request, decomposes it into the `CAMPAIGN_TEMPLATE` phase/objective hierarchy, and **pre-wires which Tier-2 lane orchestrator + agents + skills each phase needs** — emitting an approve-ready campaign plan (the `/plan-campaign` skill). It **PLANS** the routing; once you approve, the **main session (Tier-1) executes** by invoking the named lane orchestrators (two-levels-max preserved — the planner never dispatches them). See `.claude/KIT.md` §2.
...
- **Campaign planning:** `plan-campaign` (turn a user mandate into an approve-ready campaign plan — reformulated request + phase/objective decomposition + the routing map + a paste-ready ROADMAP `# CYCLE N` section; the `/plan-campaign` command and the procedure `agent-campaign-plan-orchestrator` runs).

### Agents (`@name`)
- **Orchestrators** (hold the `Agent` tool, **`opus` + `effort: high`**, each with a linked roster — see `.claude/KIT.md` §2): **Tier-0 planner** — `agent-campaign-plan-orchestrator` (plan-mode campaign planner — reformulate the mandate, decompose into phases, pre-wire which lane orchestrator/agents/skills each phase needs; it PLANS routing, Tier-1 executes after approval); **per-lane (Tier-2)** — `re-cleanroom-orchestrator` (general dirty→spec RE), `network-stack-orchestrator` (layer 02), `assets-pipeline-orchestrator` (layer 03), `client-core-orchestrator` (layer 04), `godot-client-orchestrator` (layer 05), `tooling-orchestrator` (the `.claude/` kit itself), `quality-gate-orchestrator` (pre-commit validation); **Campaign 2** — `re-comprehension-orchestrator` (READONLY comprehension), `re-annotation-orchestrator` (parallel IDB WRITE — fans out writers concurrently, Tier-3 worker `re-ida-annotator`). For a single-deliverable task, delegate straight to the worker; route an orchestrator only for a multi-lane objective.
