...
- **Terrain texture:** cell `.ted` `TextureIndexGrid` byte → cell `.map` `TERRAIN/BUILDING TEXTURES[idx-1].intTexId` → `bgtexture.LST[id]` → `data/map000/texture/<rel>.dds`. Textures are **global under `map000`** for all areas. (`bgtexture.LST` is the binary runtime index: u32 count + 48-byte records; `bgtexture.txt` is an authoring mirror only, not the live path — see `Docs/RE/formats/bgtexture_lst.md` and `Docs/RE/specs/vfs_overview.md §4`.)
...
- **Collision:** `.sod` = 2D XZ wall segments (segment-intersection, NOT ray-parity). Ground height from `.ted` **per-triangle plane interpolation** (NOT 4-corner bilinear lerp — the sampler finds the sub-quad triangle then evaluates its plane equation; see `Docs/RE/formats/terrain.md §5.4a`).
